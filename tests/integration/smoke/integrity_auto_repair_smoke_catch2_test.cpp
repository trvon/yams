...
TEST_CASE("IntegrationSmoke.IntegrityAutoRepair_FromBackupDir", "[smoke][integrationsmoke]") {
    ...
    CHECK(result.status == VerificationStatus::Repaired);
...
TEST_CASE("IntegrationSmoke.IntegrityAutoRepair_FromP2PDir", "[smoke][integrationsmoke]") {
    namespace fs = std::filesystem;
...
TEST_CASE("IntegrationSmoke.VerifyCLIRepairsFromBackupFlag", "[smoke][integrationsmoke]") {
    namespace fs = std::filesystem;
    ...
    REQUIRE_NOTHROW(verifyApp.parse(argc, argv.data()));

    REQUIRE(attemptRepair);
    REQUIRE(hashArg == hash);
