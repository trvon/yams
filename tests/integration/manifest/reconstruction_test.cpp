...
protected:
    ...
    void SetUp() override {
        ...
        ManifestManager::Config manifestConfig{
    ...
    std::filesystem::path createTestFile(const std::string& content, const std::string& filename = "test.txt") {
...
TEST_F(ManifestIntegrationTest, EndToEndFileProcessing) {
    ...
    spdlog::info("Chunked file into {} chunks, total size: {}", 
                chunks.size(), fileInfo.size);
    ...
    spdlog::info("Manifest serialized to {} bytes ({:.2f}% of original file)", 
                serializedManifest.size(), 
                100.0 * serializedManifest.size() / fileInfo.size);
    ...
    auto reconstructResult = manifestManager->reconstructFile(retrievedManifest, reconstructedFile, provider);
    ...
    std::string reconContent((std::istreambuf_iterator<char>(recon)), std::istreambuf_iterator<char>());
...
TEST_F(ManifestIntegrationTest, LargeFileProcessing) {
    ...
    spdlog::info("Large file: {} bytes, {} chunks, avg chunk size: {}", 
                fileInfo.size, chunks.size(), fileInfo.size / chunks.size());
    ...
    auto reconstructResult = manifestManager->reconstructFile(manifest, reconstructedFile, provider);
...
TEST_F(ManifestIntegrationTest, MultipleFileDeduplication) {
    ...
    for (int fileNum = 0; fileNum < 5; ++fileNum) {
        ...
        for (int i = 0; i < 20; ++i) {
            content += "Unique line " + std::to_string(i) + " for file " + std::to_string(fileNum) + ". ";
        }
    ...
    for (size_t i = 0; i < files.size(); ++i) {
        ...
        auto reconstructResult = manifestManager->reconstructFile(manifests[i], reconstructedFile, provider);
    ...
    spdlog::info("Storage stats: {} objects, {} bytes", 
                storageStats.totalObjects.load(), storageStats.totalBytes.load());
    ...
    double compressionRatio = static_cast<double>(storageStats.totalBytes.load()) / totalOriginalSize;
