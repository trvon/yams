...
TEST_F(TaskManagementTest, TaskTrackingWorkflow) {
    ...
    for (const auto& task : tasks) {
        auto content =
            createTaskDocument(task.id, task.title, "Task description for " + task.title);
        ...
        metadataRepo_->setMetadata(docId, "priority",
                                   MetadataValue(static_cast<double>(task.priority)));
    ...
    metadataRepo_->setMetadata(taskDocIds["TASK-001"], "started",
                               MetadataValue(getCurrentTimestamp()));
    ...
    metadataRepo_->setMetadata(taskDocIds["TASK-001"], "completed",
                               MetadataValue(getCurrentTimestamp()));
...
TEST_F(TaskManagementTest, StatusUpdateWorkflow) {
    ...
    for (size_t i = 0; i < transitions.size(); ++i) {
        ...
        std::string value =
            transitions[i].from + "->" + transitions[i].to + "@" + transitions[i].timestamp;
...
TEST_F(TaskManagementTest, TaskDependencies) {
    ...
    std::vector<TaskDep> tasks = {{"TASK-A", "Database Schema", {}},
...
TEST_F(TaskManagementTest, ReportingAndAnalytics) {
    ...
    std::vector<TaskData> tasks = {{"RPT-001", "completed", "alice", 8.0, 6.0},
    ...
    for (const auto& result : allTasks) {
        ...
        if (estimated.has_value())
            totalEstimated += estimated->toDouble();
        if (spent.has_value())
