...
TEST_F(MultiCommandTest, MetadataFilteringWorkflow) {
    ...
    std::vector<DocInfo> docs = {
        {"doc1.txt", "pending", 1, "urgent"},
        {"doc2.txt", "active", 2, "normal"},
        {"doc3.txt", "completed", 1, "urgent"},
        {"doc4.txt", "pending", 3, "low"},
        {"doc5.txt", "active", 1, "urgent"},
        {"doc6.txt", "completed", 2, "normal"},
        {"doc7.txt", "pending", 2, "normal"},
        {"doc8.txt", "active", 3, "low"},
        {"doc9.txt", "completed", 3, "low"},
        {"doc10.txt", "pending", 1, "urgent"}
    };
    ...
    for (const auto& doc : docs) {
        ...
        metadataRepo_->setMetadata(docId, "priority", MetadataValue(static_cast<double>(doc.priority)));
...
TEST_F(MultiCommandTest, ComplexQueryChaining) {
    ...
    std::vector<Document> documents = {
        ...
        {generator_->generateTextDocument(1024), "article", "charlie", 2023, 4.2, {"research", "ml"}},
        {generator_->generateTextDocument(1024), "guide", "alice", 2024, 4.9, {"tutorial", "ai"}},
        {generator_->generateTextDocument(1024), "report", "diana", 2022, 3.5, {"analysis", "stats"}},
...
TEST_F(MultiCommandTest, ErrorPropagation) {
    ...
    auto invalidUpdate = metadataRepo_->setMetadata(
        docResult->value()->id,
        "", // Invalid empty key
        MetadataValue("value")
    );
