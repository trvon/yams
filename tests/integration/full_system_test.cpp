...
protected:
    ...
    void SetUp() override {
        ...
        auto result = ContentStoreBuilder().withConfig(config).build();
    ...
    fs::path createPatternFile(const std::string& name, const std::string& pattern,
                               size_t repeats) {
        fs::path filePath = testDir / name;
...
TEST_F(FullSystemIntegrationTest, LargeFileHandling) {
    ...
    double storeMBps = (100.0 * 1024 * 1024) /
                       std::chrono::duration_cast<std::chrono::milliseconds>(storeTime).count() /
                       1000;
    double retrieveMBps =
        (100.0 * 1024 * 1024) /
    ...
    for (int t = 0; t < numThreads; ++t) {
        threads.emplace_back([this, t, &successCount]() {
            for (int f = 0; f < filesPerThread; ++f) {
                std::string name =
                    "thread" + std::to_string(t) + "_file" + std::to_string(f) + ".txt";
...
TEST_F(FullSystemIntegrationTest, BatchOperations) {
    ...
    for (int i = 0; i < 20; ++i) {
        compareFiles.push_back(
            createTestFile("individual" + std::to_string(i) + ".dat", 10 * 1024 * (i + 1)));
    }
    ...
    std::cout << "Batch time: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(batchTime).count() << "ms"
              << std::endl;
    std::cout << "Individual time: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(individualTime).count()
              << "ms" << std::endl;
}
