...
TEST_F(PDFIntegrationTest, PDFMetadataExtraction) {
    ...
    std::vector<std::string> expectedMetadataKeys = {"pdf_title",    "pdf_author",  "pdf_subject",
...
TEST_F(PDFIntegrationTest, MixedDocumentTypeWorkflow) {
    ...
    std::map<std::string, bool> foundTypes = {{"pdf", false}, {"text", false}, {"markdown", false}};
...
TEST_F(PDFIntegrationTest, BatchPDFProcessing) {
    ...
    std::cout << "Batch processing " << numPDFs << " PDFs took: " << duration.count() << "ms"
              << std::endl;
