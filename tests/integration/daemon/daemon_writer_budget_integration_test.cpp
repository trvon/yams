...
TEST(ServerMultiplexIntegrationTest, WriterBudgetAllowsFastUnary) {
    ...
    if (!(enabled && std::string(enabled) == "1")) {
        GTEST_SKIP() << "Set YAMS_TEST_WRITER_BUDGET=1 and start daemon with YAMS_SERVER_WRITER_BUDGET_BYTES";
    }

    yams::daemon::ClientConfig cfg;
    cfg.requestTimeout = 5s; cfg.headerTimeout = 3s; cfg.bodyTimeout = 5s; cfg.maxInflight = 64;
    ...
    auto long_stream = std::async(std::launch::async, [&]() {
        yams::daemon::SearchRequest req; req.query = "test"; req.limit = 1000; // heavy response
    ...
    ASSERT_TRUE(sres);
    (void)sres.value();
    EXPECT_LT(dt_ms, 2000) << "StatusRequest exceeded expected latency under budget: " << dt_ms << " ms";
