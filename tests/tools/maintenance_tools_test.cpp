...
TEST_F(MaintenanceToolsTest, StatsToolExecution) {
    ...
    std::string command = "/Volumes/picaso/work/tools/kronos/tools/kronos-stats " + testDir.string();
...
TEST_F(MaintenanceToolsTest, GCToolDryRun) {
    ...
    std::string command = "/Volumes/picaso/work/tools/kronos/tools/kronos-gc " + testDir.string() + " --dry-run --force";
...
TEST_F(MaintenanceToolsTest, VerifyStoredData) {
    ...
    for (const auto& hash : storedHashes) {
        ...
        EXPECT_TRUE(retrieveResult.has_value()) << "Should be able to retrieve stored block: " << hash;
...
TEST_F(MaintenanceToolsTest, ToolsHandleNonexistentPath) {
    ...
    std::string statsCommand = "/Volumes/picaso/work/tools/kronos/tools/kronos-stats " + nonexistentPath + " 2>/dev/null";
    ...
    std::string gcCommand = "/Volumes/picaso/work/tools/kronos/tools/kronos-gc " + nonexistentPath + " --dry-run 2>/dev/null";
