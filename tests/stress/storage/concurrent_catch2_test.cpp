...
TEST_CASE_METHOD(ConcurrentStorageTest, "HundredConcurrentWriters", "[stress][concurrentstoragetest]") {
    ...
    CHECK(successfulWrites.load() == numWriters * writesPerThread);

    // Check storage statistics
    auto stats = storage->getStats();
    CHECK(stats.writeOperations.load() == numWriters * writesPerThread);
...
TEST_CASE_METHOD(ConcurrentStorageTest, "RapidCreateDeleteCycles", "[stress][concurrentstoragetest]") {
    ...
    CHECK_FALSE(errors.load());
    CHECK(creates.load() == numThreads * cyclesPerThread);
    CHECK(deletes.load() == numThreads * cyclesPerThread);
...
TEST_CASE_METHOD(ConcurrentStorageTest, "LatencyUnderLoad", "[stress][concurrentstoragetest]") {
    ...
    for (std::size_t workers : workerSweep) {
        ...
        if (baseline == 0.0) {
            ...
            if (throughput < failThreshold) {
                FAIL_CHECK("connection pool regression for " + std::to_string(workers) +
                           " workers (" + std::to_string(throughput) + " < " +
                           std::to_string(failThreshold) + ")");
...
TEST_CASE_METHOD(ConcurrentStorageTest, "VariableObjectSizesConcurrent", "[stress][concurrentstoragetest]") {
    ...
    CHECK(successfulOps.load() == numThreads * opsPerThread);
    CHECK(failedOps.load() == 0u);
    CHECK(corruptedReads.load() == 0u);
}

// New stress test: Burst traffic patterns
TEST_CASE_METHOD(ConcurrentStorageTest, "BurstTrafficPattern", "[stress][concurrentstoragetest]") {
...
TEST_CASE_METHOD(ConcurrentStorageTest, "LongRunningSustainedLoad", "[stress][concurrentstoragetest]") {
    ...
    CHECK(errors.load() == 0u);
    CHECK(totalOperations.load() > numThreads * 100); // Should do many operations
}
