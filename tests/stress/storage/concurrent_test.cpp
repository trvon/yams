...
TEST_F(ConcurrentStorageTest, ThousandConcurrentReaders) {
    ...
    spdlog::info("Completed {} reads in {} ms ({} reads/sec)", 
                totalReads.load(), duration.count(), readsPerSecond);
...
TEST_F(ConcurrentStorageTest, HundredConcurrentWriters) {
    ...
    spdlog::info("Completed {} writes in {} ms ({} writes/sec)", 
                totalWrites.load(), duration.count(), 
                (totalWrites.load() * 1000) / duration.count());
}
...
TEST_F(ConcurrentStorageTest, MixedReadWriteWorkload) {
    ...
    spdlog::info("Mixed workload: {} reads, {} writes in {} ms", 
                readOps.load(), writeOps.load(), duration.count());
...
TEST_F(ConcurrentStorageTest, LatencyUnderLoad) {
    ...
    spdlog::info("Existence check latencies under load - P50: {:.2f}ms, P95: {:.2f}ms, P99: {:.2f}ms",
                p50, p95, p99);
