...
| Plan | Focus |
|------|--------|
| `search_component_ablation` / `subsystem_overhead` | Search subsystem one-factor-off |
| `ingest_pipeline` | Ingest kg/vectors/gliner |
| `retrieval_load` | Concurrent load, search_type |
| `repair_ability` | Fault-kind repair |
| `ops_timeline` | Idle / drain |
| `topology_*` / `simeon_rerank` / `topology_core_ab` | Quality / topology matrices |
