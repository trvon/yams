...
public:
    MetadataBenchmark(const std::string& name) : BenchmarkBase("Metadata_" + name) { setUp(); }
...
BENCHMARK_F(MetadataBenchmark, SingleUpdate){MetadataBenchmark("SingleUpdate"){}

                                             size_t runIteration()
                                                 override{static size_t docIndex = 0;

const auto& docId = documentIds_[docIndex % documentIds_.size()];
docIndex++;

// Update a single metadata field
auto result = metadataRepo_->setMetadata(docId, "status",
...
BENCHMARK_F(MetadataBenchmark, BulkUpdate){MetadataBenchmark("BulkUpdate"){}

                                           size_t runIteration()
                                               override{const size_t batchSize = 100;
...
BENCHMARK_F(MetadataBenchmark, QuerySingleField){MetadataBenchmark("QuerySingleField"){}

                                                 void setUp(){MetadataBenchmark::setUp();
...
BENCHMARK_F(MetadataBenchmark, ComplexQuery){MetadataBenchmark("ComplexQuery"){}
...
BENCHMARK_F(MetadataBenchmark, GetAllMetadata){MetadataBenchmark("GetAllMetadata"){}
...
// Remove metadata
BENCHMARK_F(MetadataBenchmark, RemoveMetadata){MetadataBenchmark("RemoveMetadata"){}
...
BENCHMARK_F(MetadataBenchmark, ConcurrentOperations){MetadataBenchmark("ConcurrentOperations"){}

                                                     size_t runIteration()
                                                         override{const size_t numThreads = 4;
    ...
    threads.emplace_back([this, t, &successCount, &failCount]() {
        ...
        for (size_t i = 0; i < 25; ++i) {
            ...
            switch (op) {
                ...
                case 1: // Get
                {
                    auto getResult = metadataRepo_->getMetadata(documentIds_[docIdx],
                                                                "thread_" + std::to_string(t));
                    result = getResult.has_value() ? Result<void>::ok()
                                                   : Result<void>::error(getResult.error());
                } break;

                case 2: // Remove
                    result = metadataRepo_->removeMetadata(documentIds_[docIdx],
                                                           "thread_" + std::to_string(t));
                    break;
            }
