...
class RegressionChecker:
    ...
    def compare_benchmarks(
        ...
        for name, current_value in current_benchmarks.items():
            ...
            change = (
                (current_value["value"] - baseline_value["value"])
                / baseline_value["value"]
            ) * 100

            # Determine if regression based on unit type
            is_time_based = any(
                unit in current_value.get("unit", "")
            ...
            current_metrics = current_value.get("metrics", {})
            baseline_metrics = baseline_value.get("metrics", {})
            for metric_name, baseline_metric in baseline_metrics.items():
                ...
                if not isinstance(current_metric, (int, float)):
                    continue
                metric_comparison = self._metric_regression(
                    name,
                    metric_name,
                ...
                if metric_comparison["is_regression"]:
                    has_regression = True
                    self.regressions.append(metric_comparison)
                elif metric_comparison["is_improvement"]:
                    self.improvements.append(metric_comparison)

        return has_regression, comparisons

    def _coerce_entry(self, name: str, entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _extract_history_benchmarks(
        ...
        for name, rows in history.items():
            if isinstance(rows, list) and rows:
                ...
                if isinstance(latest, dict):
                    benchmarks[name] = self._coerce_entry(name, latest)
            elif isinstance(rows, dict):
                benchmarks[name] = self._coerce_entry(name, rows)
        return benchmarks

    def _extract_summary_benchmarks(
        ...
        for name, details in summary.items():
            ...
            for key, value in details.items():
                if key in {"latest", "average", "unit"}:
                    continue
                if isinstance(value, (int, float)):
                    normalized["metrics"][key] = float(value)
            benchmarks[name] = normalized
        return benchmarks

    def _metric_regression(
        self, name: str, metric_name: str, current_value: float, baseline_value: float
    ) -> Dict[str, Any] | None:
        if baseline_value == 0:
            return None
    ...
    def _extract_benchmarks(self, data: Dict) -> Dict:
        ...
        if "benchmarks" in data:
            ...
            # Summary format
            benchmarks = self._extract_summary_benchmarks(data["summary"])
        elif "history" in data:
            # BenchmarkTracker history format
            benchmarks = self._extract_history_benchmarks(data["history"])
    ...
    def print_report(self, comparisons: List[Dict]):
        ...
        print("-" * 80)
        print(
            f"{'Benchmark':<40} {'Baseline':>12} {'Current':>12} {'Change':>10} {'Status':>10}"
        )
        print("-" * 80)

        for comp in sorted(
            comparisons, key=lambda x: x["change_percent"], reverse=True
        ):
            status = (
                "❌ REGRESS"
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=None, help="Output JSON report path"
    )
    parser.add_argument(
        "--fail-on-regression",
        action="store_true",
    ...
    try:
        ...
        has_regression, comparisons = checker.compare_benchmarks(
