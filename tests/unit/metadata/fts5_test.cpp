...
protected:
    void SetUp() override {
        ...
        dbPath = std::filesystem::temp_directory_path() / "yams_fts5_test.db";
...
TEST_F(FTS5Test, CheckFTS5Support) {
    ...
    ASSERT_TRUE(openResult.has_value()) << "Failed to open database: " 
                                        << openResult.error().message;
    
    // Check if FTS5 is available
    auto result = db.hasFTS5();
    
    ASSERT_TRUE(result.has_value()) << "Failed to check FTS5 support: " 
...
TEST_F(FTS5Test, FallbackBehavior) {
    ...
    if (!result.value()) {
        ...
        auto searchStmt = db.prepare(
            "SELECT * FROM documents WHERE content LIKE '%test%'"
        );
        ASSERT_TRUE(searchStmt.has_value());
        ...
        ASSERT_TRUE(stepResult.has_value());
        bool hasResults = stepResult.value();
        ASSERT_TRUE(hasResults) 
            << "Fallback LIKE search returned no results";
...
TEST_F(FTS5Test, ReportFTS5Configuration) {
    ...
    auto compileStmt = db.prepare(
        "SELECT name FROM pragma_compile_options WHERE name LIKE '%FTS%'"
    );
    ...
    spdlog::info("========================================");
    spdlog::info("FTS5 Support Status: {}", 
                fts5Result.value() ? "ENABLED ✓" : "DISABLED ✗");
    spdlog::info("========================================");
}