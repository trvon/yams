...
TEST_CASE("QueryLiteralExtractor::extract() with null AST", "[search][literal_extractor][catch2]") {
    ...
    REQUIRE_THAT(result.estimatedSelectivity,
                 Catch::Matchers::WithinAbs(1.0f, 1e-6f));
}

TEST_CASE("QueryLiteralExtractor::extract() with single TermNode", "[search][literal_extractor][catch2]") {
    TermNode term("hello");
...
TEST_CASE("QueryLiteralExtractor::extract() with PhraseNode", "[search][literal_extractor][catch2]") {
...
TEST_CASE("QueryLiteralExtractor::extract() with FieldNode", "[search][literal_extractor][catch2]") {
...
TEST_CASE("QueryLiteralExtractor::extract() with GroupNode", "[search][literal_extractor][catch2]") {
...
TEST_CASE("QueryLiteralExtractor scoring influences mostSelective", "[search][literal_extractor][catch2]") {
...
TEST_CASE("QueryLiteralExtractor::estimateSelectivity via extract()", "[search][literal_extractor][catch2]") {
    ...
    SECTION("Uppercase reduces selectivity by 0.7x") {
        ...
        const auto result = QueryLiteralExtractor::extract(&term);
        REQUIRE_THAT(result.estimatedSelectivity,
                     Catch::Matchers::WithinAbs(0.35f, 1e-6f));
    }
}
