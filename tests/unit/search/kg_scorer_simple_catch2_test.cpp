// Catch2 tests for KG Scorer (Simple implementation)
// Migrated from GTest: kg_scorer_simple_test.cpp
...
TEST_CASE("SimpleKGScorer - ScoresEntityAndStructuralOverlap", "[search][kg_scorer][catch2]") {
    ...
    REQUIRE(scores.count(std::to_string(did1.value())) == 1u);
    REQUIRE(scores.count(std::to_string(did2.value())) == 1u);
    REQUIRE(scores.count("abc") == 1u);

    auto s1 = scores[std::to_string(did1.value())];
    // Expect both components > 0 due to alias match (A) and neighbor overlap (B)
    CHECK(s1.entity > 0.0f);
    CHECK(s1.structural > 0.0f);

    auto s2 = scores[std::to_string(did2.value())];
    CHECK(s2.entity == 0.0f);
    CHECK(s2.structural == 0.0f);

    auto s3 = scores["abc"]; // non-numeric id should produce zero scores, but still present
    CHECK(s3.entity == 0.0f);
    CHECK(s3.structural == 0.0f);
