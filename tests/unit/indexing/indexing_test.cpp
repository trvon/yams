...
protected:
    ...
    void SetUpWithDatabase() {
        ...
        if (!initResult) {
            throw std::runtime_error("Failed to initialize migration system: " +
...
TEST_F(IndexingTest, ContentChunking) {
    ...
    public:
        std::vector<ContentChunk> chunkContent(const std::string& content,
            ...
            if (content.size() <= config.chunkSize) {
                ...
                while (offset < content.size()) {
                    ...
                    if (offset + config.overlapSize >= content.size())
                        break;
                }
            }
            return chunks;
        ...
        std::string preprocessText(const std::string& text) override { return text; }
    ...
    std::string largeContent =
        "This is a very long content that should be split into multiple chunks";
...
TEST_F(IndexingTest, BatchDocumentIndexing) {
    ...
    std::vector<std::filesystem::path> paths = {testDir_ / "test1.txt", testDir_ / "test2.md",
                                                testDir_ / "test3.cpp"};

    // Index batch with progress callback
    size_t progressCalls = 0;
    auto progressCallback = [&progressCalls](size_t current, size_t total,
...
TEST_F(IndexingTest, IndexingPipeline) {
    ...
    auto taskIds = pipeline.queueDocuments(
        {testDir_ / "test1.txt", testDir_ / "test2.md", testDir_ / "test3.cpp"});
