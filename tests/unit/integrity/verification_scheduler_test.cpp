...
TEST_F(VerificationSchedulerTest, AddMultipleBlocks) {
    std::vector<BlockInfo> blocks = {
        createTestBlock("hash1"),
        createTestBlock("hash2"), 
...
TEST_F(VerificationSchedulerTest, PriorityOrdering) {
    ...
    auto lowPriority = createTestBlock("hash_low", std::chrono::hours{1}, 0, 1024, 0);    // Age: 1 hour
    auto mediumPriority = createTestBlock("hash_med", std::chrono::hours{24}, 1, 1024, 5); // Age: 1 day, 1 failure, some access
    auto highPriority = createTestBlock("hash_high", std::chrono::hours{48}, 3, 1024, 10); // Age: 2 days, 3 failures, high access
...
TEST_F(VerificationSchedulerTest, StrategyChange) {
    ...
    auto block2 = createTestBlock("hash2", std::chrono::hours{1}, 3, 512, 0);    // New, small, failures
...
TEST_F(VerificationSchedulerTest, LargeNumberOfBlocks) {
    ...
    for (size_t i = 0; i < numBlocks; ++i) {
        blocks.push_back(createTestBlock(
            "hash" + std::to_string(i),
            std::chrono::hours{ageDist(rng)},
            failureDist(rng),
            1024,
            accessDist(rng)
        ));
        ...
        threads.emplace_back([this, t, blocksPerThread]() {
            for (int i = 0; i < blocksPerThread; ++i) {
                auto block = createTestBlock(
                    "thread" + std::to_string(t) + "_block" + std::to_string(i),
