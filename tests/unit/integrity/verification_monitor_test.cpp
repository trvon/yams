...
protected:
    ...
    VerificationResult createResult(const std::string& hash, VerificationStatus status, size_t size = 1024) {
...
TEST_F(VerificationMonitorTest, ErrorRateThreshold) {
    ...
    for (int i = 0; i < 10; ++i) {
        monitor->recordVerification(createResult("pass" + std::to_string(i), VerificationStatus::Passed));
    }
    
    // Add some failures to exceed threshold
    for (int i = 0; i < 5; ++i) {
        monitor->recordVerification(createResult("fail" + std::to_string(i), VerificationStatus::Failed));
    }
...
TEST_F(VerificationMonitorTest, RateCalculationOverTime) {
    ...
    for (int i = 0; i < numVerifications; ++i) {
        monitor->recordVerification(createResult("hash" + std::to_string(i), VerificationStatus::Passed));
...
TEST_F(VerificationMonitorTest, ThreadSafetyTest) {
    ...
    for (int t = 0; t < numThreads; ++t) {
        threads.emplace_back([this, t, verificationsPerThread]() {
            for (int i = 0; i < verificationsPerThread; ++i) {
                VerificationStatus status = (i % 4 == 0) ? VerificationStatus::Failed : VerificationStatus::Passed;
                auto result = createResult(
                    "thread" + std::to_string(t) + "_hash" + std::to_string(i), 
                    status
                );
                monitor->recordVerification(result);
                ...
                if (i % 10 == 0) {
                    monitor->recordRepairAttempt("repair_" + std::to_string(t) + "_" + std::to_string(i), i % 2 == 0);
                }
            }
        });
    }
    ...
    EXPECT_EQ(metrics.totalErrors, numThreads * (verificationsPerThread / 4)); // Every 4th is failed
...
TEST_F(VerificationMonitorTest, LongRunningMonitoring) {
    ...
    for (int cycle = 0; cycle < cycles; ++cycle) {
        ...
        for (int i = 0; i < verificationsPerCycle; ++i) {
            VerificationStatus status = (i % 5 == 0) ? VerificationStatus::Failed : VerificationStatus::Passed;
            monitor->recordVerification(createResult(
                "cycle" + std::to_string(cycle) + "_" + std::to_string(i), 
                status
            ));
        }
