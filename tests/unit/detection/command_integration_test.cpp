...
TEST_F(CommandIntegrationTest, FileTypeDetectorIntegration_ValidConfig) {
    ...
    EXPECT_NO_THROW({
        ...
        std::string category = detection::FileTypeDetector::instance().getFileTypeCategory("application/pdf");
...
TEST_F(CommandIntegrationTest, FileTypeDetectorIntegration_MissingJSON) {
    ...
    EXPECT_NO_THROW({
        bool isText = detection::FileTypeDetector::instance().isTextMimeType("text/plain");
        std::string category = detection::FileTypeDetector::instance().getFileTypeCategory("image/jpeg");
        EXPECT_TRUE(isText);
        EXPECT_FALSE(category.empty());
    });
}
...
TEST_F(CommandIntegrationTest, FileTypeDetectorIntegration_FileDetection) {
    ...
    for (const auto& testFile : testFiles) {
        ...
        }) << "Should handle file detection gracefully for " << testFile;
    }
}
...
TEST_F(CommandIntegrationTest, CommandSimulation_StatsWithFileTypes) {
    ...
    EXPECT_NO_THROW({
        ...
        for (const auto& testFile : testFiles) {
            ...
            std::string fileType = detection::FileTypeDetector::instance().getFileTypeCategory(mimeType);
            ...
            EXPECT_TRUE(isBinary || !isBinary) << "Binary classification should work for " << testFile;
        }
    });
}
