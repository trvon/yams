...
struct CliTestHelper {
    ...
    CliTestHelper() {
        tempDir = yams::test::make_temp_dir("yams_cli_delete_catch2_test_");
...
TEST_CASE("DeleteCommand - dry run missing target falls back without crashing",
          "[cli][delete][catch2]") {
    CliTestHelper helper;
    CaptureStdout capture;

    const int rc =
        helper.runCommand({"yams", "delete", "--dry-run", "--force", "missing-document"});
