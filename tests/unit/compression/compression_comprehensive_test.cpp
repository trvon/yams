...
protected:
    ...
    std::vector<std::byte> generateTestData(size_t size, const std::string& pattern) {
        ...
        if (pattern == "zeros") {
            ...
            for (size_t i = 0; i < size; ++i) {
                if (i % 4 == 0) data[i] = std::byte{0x00};
                else if (i % 4 == 1) data[i] = std::byte{0xFF};
                else if (i % 4 == 2) data[i] = std::byte{0xAA};
                else data[i] = std::byte{0x55};
            }
        }
...
TEST_F(CompressionComprehensiveTest, CompressionLevels) {
    ...
    for (uint8_t level = 1; level <= 9; ++level) {
        ...
        auto decompressed = mockZstd_->decompress(compressed.value().data);
        ASSERT_TRUE(decompressed.has_value()) << "Failed to decompress level " << static_cast<int>(level);
        EXPECT_EQ(decompressed.value(), data) << "Data mismatch at level " << static_cast<int>(level);
    }
}
...
TEST_F(CompressionComprehensiveTest, CompressAlreadyCompressed) {
    ...
    double ratio = static_cast<double>(compressed2.value().originalSize) / 
                   compressed2.value().compressedSize;
