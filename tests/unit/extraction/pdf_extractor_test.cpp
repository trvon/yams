...
protected:
    ...
    void TearDown() override { fixtureManager_->cleanupTempDirectories(); }
...
TEST_F(PdfExtractorTest, ExtractMetadataComplete) {
    ...
    std::vector<std::string> expectedFields = {"title",   "author",   "subject",   "keywords",
                                               "creator", "producer", "page_count"};

    for (const auto& field : expectedFields) {
        EXPECT_TRUE(metadata.find(field) != metadata.end()) << "Missing metadata field: " << field;
    }
}
