...
public:
    ...
    std::vector<uint8_t> generatePDF(size_t pages) {
        ...
        for (size_t i = 0; i < pages; ++i) {
            ...
            // Content stream
            objNum++;
            std::string content = "BT /F1 12 Tf 100 700 Td (Page " + std::to_string(i + 1) + ": " + generateLoremIpsum(20) + ") Tj ET";
            pdf += std::to_string(objNum) + " 0 obj\n";
    ...
    // Metadata generators
    std::map<std::string, metadata::MetadataValue> generateMetadata(
        size_t numFields,
    ...
    std::string generateJSONObject(size_t depth, size_t breadth, size_t currentDepth) {
        if (currentDepth >= depth) {
            ...
            switch (typeDist(rng_)) {
                case 0: return "\"" + generateWord() + "\"";
                case 1: return std::to_string(std::uniform_int_distribution<int>(0, 100)(rng_));
                case 2: return std::uniform_int_distribution<int>(0, 1)(rng_) ? "true" : "false";
            }
        }
