...
class ZstdConan(ConanFile):
    ...
    def source(self):
        ...
        # Patch: Remove symlinks in test directories to avoid Docker volume issues
        # These are only needed for test programs which we typically disable in fuzzing builds
        self._remove_test_symlinks()

    def _remove_test_symlinks(self):
        """Remove problematic symlinks in test directories that fail on macOS Docker volumes"""
        test_dirs = [
            os.path.join(self.source_folder, "tests", "cli-tests", "bin"),
            os.path.join(self.source_folder, "programs"),
        ]
                    ...
                    for name in files + dirs:
                        ...
                        try:
                            if os.path.islink(path):
                                self.output.info(f"Removing symlink: {path}")
                                os.unlink(path)
                        except (OSError, IOError) as e:
                            # Log but don't fail - symlink removal is best-effort
                            self.output.info(f"Note: Could not remove {path}: {e}")
