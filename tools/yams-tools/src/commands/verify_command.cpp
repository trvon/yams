...
public:
    ...
    void setupOptions(CLI::App& app) override {
        ...
        verify->add_option("-h,--hash", specificHash_,
                          "Verify specific content hash");
        
        verify->add_flag("-f,--full", fullScan_,
                        "Full scan of all blocks");
        
        verify->add_option("-s,--sample", sampleRate_,
                          "Sample rate percentage (1-100)")
             ->default_val(100)
             ->check(CLI::Range(1, 100));
        
        verify->add_option("-j,--jobs", parallelJobs_,
                          "Number of parallel verification jobs")
             ->default_val(4);
        
        verify->add_flag("-r,--repair", attemptRepair_,
                        "Attempt to repair corrupted blocks");
        
        verify->add_option("--report-interval", reportIntervalHours_,
...
private:
    ...
    int performScan(integrity::IntegrityVerifier& verifier,
                    storage::ReferenceCounter& refCounter) {
        log(fullScan_ ? "Starting full integrity scan..." : 
        ...
        log("Selected " + std::to_string(blocksToVerify.size()) + 
            " out of " + std::to_string(totalBlocks) + " blocks for verification");
        ...
        double successRate = blocksToVerify.size() > 0 ?
            (static_cast<double>(passed.load()) / blocksToVerify.size() * 100) : 0;
        ...
        if (failed.load() > 0 || corrupted.load() > 0 || missing.load() > 0) {
            ...
            if (failures.size() < (failed.load() + corrupted.load() + missing.load())) {
                std::cout << "  ... and " << ((failed.load() + corrupted.load() + 
                                             missing.load()) - failures.size())
                         << " more\n";
            }
        }
