...
static const char* kName = "yams_fuzz_abi_test";
static const char* kVersion = "0.0.1";
static const char* kManifest =
    "{"
    "\"name\":\"yams_fuzz_abi_test\","
    "\"version\":\"0.0.1\","
    "\"abi\":1,"
    "\"interfaces\":[{\"id\":\"fuzz_iface_v1\",\"version\":1}]"
    "}";
...
static int iface_ping(int x) {
    return x ^ 0x5A5A;
}

static struct FuzzIfaceV1 kIface = {iface_ping};
...
YAMS_PLUGIN_API int yams_plugin_get_interface(const char* iface_id, uint32_t version,
    ...
    if (version == 1 && strcmp(iface_id, "fuzz_iface_v1") == 0) {
        *out_iface = &kIface;
        return YAMS_PLUGIN_OK;
    }
    *out_iface = NULL;
...
YAMS_PLUGIN_API int yams_plugin_get_health_json(char** out_json) {
    ...
    const char* payload = "{\"ok\":true,\"plugin\":\"yams_fuzz_abi_test\"}";
