...
public:
    explicit ScopedEnv(const std::vector<std::pair<std::string, std::optional<std::string>>>& kvs) {
        for (const auto& [k, v] : kvs) {
            ...
            if (prev)
                prev_.emplace(k, std::string(prev));
            else
                prev_.emplace(k, std::nullopt);
            if (v.has_value())
                setEnvVar(k.c_str(), v->c_str());
            else
                unsetEnvVar(k.c_str());
            keys_.push_back(k);
        }
    ...
    ~ScopedEnv() {
        for (const auto& k : keys_) {
            ...
            if (it->second.has_value())
                setEnvVar(k.c_str(), it->second->c_str());
            else
                unsetEnvVar(k.c_str());
        }
    }
...
static bool tryParseQueriesJsonl(const std::vector<std::string>& lines, std::vector<Query>& out) {
    ...
    for (const auto& lineRaw : lines) {
        ...
        if (line.empty())
            continue;
        if (line.front() != '{')
            return false; // Not a JSON object line
        std::smatch m;
        Query q;
        if (std::regex_search(line, m, idRe))
            ...
            return false;
        if (std::regex_search(line, m, textRe))
            q.text = m[1].str();
        else
            return false;
...
static bool tryParseQueriesJsonObject(const std::string& content, std::vector<Query>& out) {
    ...
    for (const auto& obj : objs) {
        Query q;
        std::smatch m;
        if (std::regex_search(obj, m, idRe))
            q.id = m[1].str();
        else
            continue;
        if (std::regex_search(obj, m, textRe))
            q.text = m[1].str();
        else
            continue;
...
static void applyModeEnv(const std::string& mode,
...
static std::vector<Query> loadQueries(const std::string& path) {
    ...
    if (tryParseQueriesJsonl(lines, queries))
        return queries;

    // Fallback to JSON with "queries":[...]
    const std::string content = readFileAll(path);
    if (tryParseQueriesJsonObject(content, queries))
        return queries;

    throw std::runtime_error(
        "Failed to parse queries. Expected JSONL or JSON with a 'queries' array.");
}
...
static void printUsage(const char* prog) {
    std::cerr
        ...
        << "  --mode <m>              hybrid|keyword-only|vector-only|kg-only|fail-open-kg "
           "(default: hybrid)\n"
...
static RunConfig parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--queries")
            cfg.queriesPath = need("--queries");
        else if (a == "--cli")
            cfg.cliPath = need("--cli");
        else if (a == "--limit")
            cfg.limit = std::stoi(need("--limit"));
        else if (a == "--k")
            cfg.ks = parseKList(need("--k"));
        else if (a == "--extra-args")
            cfg.extraArgs = need("--extra-args");
        else if (a == "--mode")
            cfg.mode = need("--mode");
        else if (a == "--no-verbose-cli")
            cfg.verboseCli = false;
        else if (a == "--output")
            cfg.outputPath = need("--output");
        else if (a == "--help" || a == "-h") {
            printUsage(argv[0]);
            std::exit(0);
        } else {
    ...
    if (cfg.queriesPath.empty())
        throw std::runtime_error("Missing required --queries <path>");
    return cfg;
}
