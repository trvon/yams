...
### Added
- `yams list --metadata-values` for showing unique metadata values with counts (useful for work-item discovery).
...
- Unique task selection guidance in AGENTS workflow (metadata search + list values).
...
### Documentation
- Updated YAMS skill guide with unique work-item discovery and tagged search examples.
