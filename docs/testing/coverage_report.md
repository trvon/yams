...
|--------------------|--------|-----------------------|---------------------------------------------|
| `yams::api::IContentStore`, `ContentStore` | `include/yams/api/content_store.h`, `src/api/content_store_impl.cpp` | `tests/unit/api/content_store_catch2_test.cpp`, `tests/unit/api/raw_text_storage_catch2_test.cpp` | Not-ready backend injection, remote fallback refusal, partial add rollback. |
...
| `CompressedStorageEngine` | `include/yams/storage/compressed_storage_engine.h`, `src/storage/compressed_storage_engine.cpp` | `tests/unit/storage/compressed_storage_engine_catch2_test.cpp`, `tests/unit/storage/compressed_storage_stats_catch2_test.cpp` | Corrupt compression header and raw fallback invariants. |
| `IStorageBackend`, `FilesystemBackend`, `URLBackend` | `include/yams/storage/storage_backend.h`, `src/storage/url_backend.cpp` | `tests/unit/storage/storage_backend_catch2_test.cpp` | URL backend direct failure taxonomy, timeout/retry behavior, remote listing bounds. |
| `StorageBackendFactory` | `include/yams/storage/storage_backend.h`, `src/storage/storage_backend_factory.cpp` | `tests/unit/storage/storage_backend_factory_catch2_test.cpp` | Missing plugin, bad URI, S3 fallback policy, credential redaction. |
| `StorageBackendEngineAdapter` | `include/yams/storage/storage_backend_engine_adapter.h`, `src/storage/storage_backend_engine_adapter.cpp` | `tests/unit/storage/storage_backend_engine_adapter_catch2_test.cpp` | Null backend, failed store/retrieve/remove accounting, `exists()` exception conversion, and size list/retrieve failures are covered. Remaining: remote-specific size strategy/list avoidance. |
...
| `StorageBootstrapDecision`, `resolveStorageBootstrapDecision` | `include/yams/storage/storage_runtime_resolver.h`, `src/storage/storage_runtime_resolver.cpp` | `tests/unit/storage/storage_runtime_resolver_catch2_test.cpp` | Shared-store mode must not silently fork to local fallback. |
...
| `PluginBackendProxy`, plugin loader helpers | `src/storage/object_storage_plugin_loader.cpp` | No obvious direct test | `dlopen` failure, missing symbols, plugin init error, unload lifetime. |
