...
The mirror connects to Forgejo SSH at `git.trevon.dev:2222` (the former
`ssh.git.trevon.dev` endpoint no longer resolves). The workflow currently disables SSH host-key verification, so
