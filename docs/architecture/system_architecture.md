...
YAMS operates on a client-daemon architecture. The core logic runs in a persistent background daemon (`yamsd`), which is accessed by user-facing clients (such as the CLI or an AI coding assistant) over low-latency IPC.
...
graph TD
    ...
    subgraph Clients ["Client Layer"]
        ...
        MCP["MCP Host (AI Assistant)"]:::client
    end

    subgraph DaemonProcess ["YAMS Daemon Process (yamsd)"]
        IPC["IPC Socket (Protobuf/stdio)"]:::daemon
    ...
    %% Connections
    CLI -->|Protobuf Stream| IPC
    MCP -->|JSON-RPC stdio| IPC
    IPC --> WC
    WC --> SM
...
### 1. IPC & Concurrency
* **IPC Socket**: Multiplexes client connections using a streaming protobuf protocol (for the CLI) or standard JSON-RPC stdio (for the MCP server).
