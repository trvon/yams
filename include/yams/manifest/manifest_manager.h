...
struct Manifest {
    ...
    auto operator<=>(const Manifest& other) const {
        return std::tie(version, fileHash, fileSize, originalName, mimeType, 
                       createdAt, modifiedAt, chunks, checksum, compression, 
                       uncompressedSize) <=> 
               std::tie(other.version, other.fileHash, other.fileSize, 
                       other.originalName, other.mimeType, other.createdAt, 
                       other.modifiedAt, other.chunks, other.checksum, 
                       other.compression, other.uncompressedSize);
    }
...
public:
    ...
    // Core operations
    virtual Result<Manifest> createManifest(
    ...
    // File reconstruction
    virtual Result<void> reconstructFile(
        const Manifest& manifest,
        const std::filesystem::path& outputPath,
        IChunkProvider& provider) const = 0;
    
    // Async operations
    virtual std::future<Result<void>> reconstructFileAsync(
        const Manifest& manifest,
        const std::filesystem::path& outputPath,
        IChunkProvider& provider) const = 0;
    ...
    virtual Result<bool> verifyFileIntegrity(
        const Manifest& manifest,
...
public:
    ...
    Result<Manifest> createManifest(
        const FileInfo& fileInfo,
    ...
    std::future<Result<void>> reconstructFileAsync(
        const Manifest& manifest,
        const std::filesystem::path& outputPath,
        IChunkProvider& provider) const override;
    ...
    Result<bool> verifyFileIntegrity(
        const Manifest& manifest,
...
// Factory function
std::unique_ptr<IManifestManager> createManifestManager(
    ManifestManager::Config config = {});

// Utility functions for batch operations
Result<std::vector<Manifest>> deserializeManifestBatch(std::span<const std::byte> data);
Result<std::vector<std::byte>> serializeManifestBatch(
    std::ranges::input_range auto&& manifests);
...
public:
    ...
    template<typename F>
    void processBatches(size_t batchSize, F&& processor) {
