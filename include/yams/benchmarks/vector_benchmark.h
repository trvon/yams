...
struct BenchmarkResult {
    ...
    // Comparison
    bool operator<(const BenchmarkResult& other) const { return mean_time_us < other.mean_time_us; }
};
...
protected:
    // Calculate recall@k
    double calculateRecallAtK(const std::vector<std::vector<std::string>>& retrieved,
                              const std::vector<std::vector<std::string>>& relevant, size_t k);

    // Calculate precision@k
    double calculatePrecisionAtK(const std::vector<std::vector<std::string>>& retrieved,
                                 const std::vector<std::vector<std::string>>& relevant, size_t k);

    // Calculate NDCG
    double calculateNDCG(const std::vector<std::vector<std::pair<std::string, double>>>& retrieved,
...
std::vector<std::string> generateSyntheticTexts(size_t count, size_t avg_length = 100);
...
double calculateSpeedup(const BenchmarkResult& baseline, const BenchmarkResult& current);
