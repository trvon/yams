...
inline constexpr auto kMinGracefulShutdownWaitTimeout =
    kWorkCoordinatorJoinTimeout + kWorkCoordinatorExtendedJoinTimeout +
...
clampGracefulShutdownWaitTimeout(std::chrono::milliseconds requested) {
    ...
    return std::max(
        requested,
        std::chrono::duration_cast<std::chrono::milliseconds>(kMinGracefulShutdownWaitTimeout));
}
