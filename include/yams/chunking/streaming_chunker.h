...
private:
    ...
    template<ChunkProcessor F>
    Result<void> processBuffer(std::span<const std::byte> buffer, 
                              StreamingContext& ctx,
                              F&& processor) {
        size_t bufferPos = 0;
    ...
    template<ChunkProcessor F>
    void emitChunk(StreamingContext& ctx, F&& processor) {
        if (ctx.accumulator.empty()) return;
    ...
    template<ChunkProcessor F>
    void finalizeChunk(StreamingContext& ctx, F&& processor) {
        if (!ctx.accumulator.empty()) {
            emitChunk(ctx, processor);
        }
    }
