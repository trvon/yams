...
struct DocumentMetadata {
    ...
    bool isEmpty() const {
        return title.empty() && author.empty() && source.empty();
    }
};
...
struct SearchFilters {
    ...
    bool hasFilters() const {
        return !content_types.empty() || !languages.empty() || 
               !categories.empty() || !tags.empty() ||
               created_after.has_value() || created_before.has_value() ||
               min_file_size.has_value() || max_file_size.has_value() ||
...
public:
    ...
    Result<std::vector<std::string>> ingestDocuments(
    ...
    Result<SearchResponse> searchWithSession(
        const SearchRequest& request,
        const std::string& session_id);
    
    // Convenience search methods
    Result<SearchResponse> semanticSearch(
        const std::string& query,
        const SearchOptions& options = {},
        const SearchFilters& filters = {});
    
    Result<SearchResponse> keywordSearch(
        const std::string& query,
        const SearchOptions& options = {},
        const SearchFilters& filters = {});
    
    Result<SearchResponse> hybridSearch(
        const std::string& query,
        const SearchOptions& options = {},
        const SearchFilters& filters = {});
    
    // Query assistance
    Result<std::vector<std::string>> getSuggestions(
        const std::string& partial_query,
        size_t max_suggestions = 10);
    
    Result<std::vector<std::string>> getRelatedQueries(
        const std::string& query,
    ...
    Result<SearchAnalytics> getAnalytics(
...
namespace result_utils {
    ...
    std::vector<TextSnippet> generateSnippets(
        ...
        size_t max_snippets = 3,
        size_t snippet_length = 150);
     ...
     */
    std::vector<SearchResult> removeDuplicates(
