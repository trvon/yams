...
public:
    ...
    virtual Result<std::string> storeText(const std::string& text,
                                          const RawTextMetadata& metadata) = 0;
    ...
    storeBatch(const std::vector<RawTextEntry>& entries) = 0;
    virtual Result<std::vector<RawTextEntry>>
    retrieveBatch(const std::vector<std::string>& ids) = 0;

    // Search operations
    virtual Result<std::vector<RawTextEntry>> searchByKeyword(const std::string& keyword,
                                                              size_t limit = 100) = 0;
    virtual Result<std::vector<RawTextEntry>>
    searchByMetadata(const std::unordered_map<std::string, std::string>& criteria) = 0;
    virtual Result<std::vector<RawTextEntry>> fuzzySearch(const std::string& query,
...
findBestMatches(const std::string& query, const std::vector<std::string>& candidates,
                float threshold = 0.7f, size_t maxResults = 10);
} // namespace fuzzy
