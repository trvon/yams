...
public:
    ...
    ContentStoreBuilder& withStorageEngine(
        std::shared_ptr<storage::IStorageEngine> engine);
    ContentStoreBuilder& withChunker(
        std::shared_ptr<chunking::IChunker> chunker);
    ContentStoreBuilder& withHasher(
        std::shared_ptr<crypto::IHasher> hasher);
    ContentStoreBuilder& withManifestManager(
        std::shared_ptr<manifest::IManifestManager> manager);
    ContentStoreBuilder& withReferenceCounter(
    ...
    static Result<std::unique_ptr<IContentStore>> createDefault(
        const std::filesystem::path& storagePath);
    
    static Result<std::unique_ptr<IContentStore>> createFromConfig(
