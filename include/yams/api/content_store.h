...
struct StoreResult {
    ...
    [[nodiscard]] double dedupRatio() const noexcept {
        return bytesStored > 0 ? static_cast<double>(bytesDeduped) / bytesStored : 0.0;
    }
};
...
public:
    ...
    virtual Result<StoreResult> store(const std::filesystem::path& path,
                                      const ContentMetadata& metadata = {},
                                      ProgressCallback progress = nullptr) = 0;

    virtual Result<RetrieveResult> retrieve(const std::string& hash,
                                            const std::filesystem::path& outputPath,
                                            ProgressCallback progress = nullptr) = 0;

    // Stream-based operations
    virtual Result<StoreResult> storeStream(std::istream& stream,
                                            const ContentMetadata& metadata = {},
                                            ProgressCallback progress = nullptr) = 0;

    virtual Result<RetrieveResult> retrieveStream(const std::string& hash, std::ostream& output,
                                                  ProgressCallback progress = nullptr) = 0;

    // Memory-based operations
    virtual Result<StoreResult> storeBytes(std::span<const std::byte> data,
    ...
    virtual Result<void> updateMetadata(const std::string& hash,
                                        const ContentMetadata& metadata) = 0;

    // Batch operations
    virtual std::vector<Result<StoreResult>>
    storeBatch(const std::vector<std::filesystem::path>& paths,
               const std::vector<ContentMetadata>& metadata = {}) = 0;

    virtual std::vector<Result<bool>> removeBatch(const std::vector<std::string>& hashes) = 0;
