...
public:
    ...
    std::unique_ptr<ProgressReporter> createSubReporter(uint64_t subTotalBytes);
...
public:
    ...
    std::shared_ptr<ProgressReporter> addOperation(const std::string& name, uint64_t totalBytes);
