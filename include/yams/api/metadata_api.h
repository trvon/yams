...
public:
     ...
     */
    template <typename... Requests> BatchMetadataResponse batch(Requests&&... requests);
...
private:
    ...
    applyFilter(const std::vector<metadata::DocumentMetadata>& documents,
    ...
    validateFields(const metadata::DocumentMetadata& metadata);

    /**
     * @brief Update statistics
     */
    void updateStatistics(const std::string& operationType, bool success,
                          std::chrono::milliseconds duration);

    /**
     * @brief Log operation for audit
     */
    void logOperation(const std::string& operation, const std::string& details, bool success);
     ...
     */
    template <typename Func> auto measureTime(Func&& func, std::chrono::milliseconds& duration);
};
