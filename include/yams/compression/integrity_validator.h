...
struct ValidationConfig {
    ...
    size_t maxValidationSize = static_cast<size_t>(10) * static_cast<size_t>(1024) *
                               static_cast<size_t>(1024); ///< Max size for full validation
...
public:
    ...
    validateCompression(yams::span<const std::byte> original, const CompressionResult& result,
     ...
     */
    [[nodiscard]] ValidationResult
    validateDecompression(yams::span<const std::byte> compressed,
                          yams::span<const std::byte> decompressed, uint32_t originalChecksum = 0,
                          ValidationType type = ValidationType::Checksum);
     ...
     * @return Validation result
     */
    [[nodiscard]] ValidationResult validateRoundTrip(yams::span<const std::byte> original,
                                                     ICompressor& compressor, uint8_t level = 0);
     ...
     */
    [[nodiscard]] bool verifyChecksum(yams::span<const std::byte> data,
     ...
     */
    [[nodiscard]] ValidationResult performDeepAnalysis(yams::span<const std::byte> compressed,
 ...
 * @return CRC32 checksum
 */
[[nodiscard]] uint32_t crc32(yams::span<const std::byte> data, uint32_t polynomial = 0xEDB88320);
 ...
 */
[[nodiscard]] bool hasCompressionArtifacts(yams::span<const std::byte> compressed,
 ...
 */
[[nodiscard]] bool validateCompressionFormat(yams::span<const std::byte> compressed,
                                             CompressionAlgorithm algorithm);
} // namespace integrity_utils
