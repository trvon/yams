...
struct TransactionRecord {
    ...
    [[nodiscard]] std::chrono::milliseconds duration() const {
        auto end = (state == TransactionState::Active) ? std::chrono::steady_clock::now() : endTime;
...
public:
     ...
     */
    explicit TransactionManager(TransactionConfig config = {},
     ...
     */
    [[nodiscard]] Result<TransactionId>
    beginTransaction(TransactionOperation operation,
     ...
     * @return Success or error
     */
    [[nodiscard]] Result<void> abortTransaction(TransactionId txId, const std::string& reason = "");
    ...
    decompressInTransaction(std::span<const std::byte> data, CompressionAlgorithm algorithm,
     ...
     */
    [[nodiscard]] std::optional<TransactionRecord> getTransaction(TransactionId txId) const;
...
public:
     ...
     */
    TransactionScope(TransactionManager& manager, TransactionOperation operation,
                     IsolationLevel isolation = IsolationLevel::ReadCommitted);
