...
struct RecoveryRequest {
     ...
     */
    static RecoveryRequest createCompressionRecovery(std::span<const std::byte> data,
     ...
     */
    static RecoveryRequest createDecompressionRecovery(std::span<const std::byte> data,
...
struct RecoveryOperationResult {
     ...
     */
    [[nodiscard]] bool isSuccessful() const noexcept { return status == RecoveryStatus::Success; }
...
public:
     ...
     */
    RecoveryManager(RecoveryConfig config, std::shared_ptr<CompressionErrorHandler> errorHandler,
    ...
    recoverCorruptedData(std::span<const std::byte> corruptedData, CompressionAlgorithm algorithm,
                         size_t expectedSize = 0);
    ...
    tryAlternativeCompression(std::span<const std::byte> data, CompressionAlgorithm failedAlgorithm,
                              uint8_t level = 0);
 ...
 */
[[nodiscard]] CompressionAlgorithm selectFallbackAlgorithm(CompressionAlgorithm failedAlgorithm,
                                                           size_t dataSize);
 ...
 */
[[nodiscard]] double estimateRecoveryProbability(RecoveryOperation operation,
 ...
 */
[[nodiscard]] RecoveryOperation analyzeFailurePattern(const CompressionError& error);
...
repairCompressedHeaders(std::span<const std::byte> data, CompressionAlgorithm algorithm);
} // namespace recovery_utils
