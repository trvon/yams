...
struct CompressionError {
     ...
     */
    static CompressionError create(ErrorCode code, ErrorSeverity severity,
...
public:
    ...
    handleError(const CompressionError& error,
     ...
     */
    static CompressionError fromException(const std::exception& ex, CompressionAlgorithm algorithm,
     ...
     */
    template <typename T>
    static CompressionError fromResult(const Result<T>& result, CompressionAlgorithm algorithm,
                                       const std::string& operation) {
        if (result.has_value()) {
            return CompressionError::create(ErrorCode::InvalidState, ErrorSeverity::Error,
                                            operation, "fromResult called with successful result");
        }

        CompressionError error = CompressionError::create(
            result.error().code, mapErrorCodeToSeverity(result.error().code), operation,
...
public:
     ...
     */
    CompressionErrorScope(CompressionErrorHandler& handler, const std::string& operation,
     ...
     */
    template <typename T>
    [[nodiscard]] RecoveryResult handleError(const CompressionError& error,
     ...
     * @return Compression error
     */
    [[nodiscard]] CompressionError createError(ErrorCode code,
