...
class IObjectStorageBackend {
 public:
  ...
  virtual std::variant<std::vector<std::uint8_t>, ErrorInfo> get(std::string_view key,
  ...
  virtual std::variant<PutResult, ErrorInfo> compose(std::string_view target,
                                                     ...
                                                     const PutOptions& opts) { return ErrorInfo{ErrorInfo::Category::Terminal, 501}; }
  ...
  // DR hooks (optional)
  virtual std::variant<bool, std::monostate> drObjectPresent(std::string_view /*target*/, std::string_view /*key*/) { return std::monostate{}; }
  virtual std::variant<std::uint64_t, std::monostate> replicationLagSeconds() { return std::monostate{}; }
};
