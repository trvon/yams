...
template<typename T>
concept BlockHashRange = std::ranges::input_range<T> && 
    std::convertible_to<std::ranges::range_value_t<T>, std::string>;

template<typename T>
concept BlockInfoRange = std::ranges::input_range<T> && 
...
public:
    ...
    Result<void> incrementBatch(R&& hashes, size_t blockSize = 0) {
    ...
    template<BlockInfoRange R>
    Result<void> incrementBatchWithSizes(R&& items) {
        auto txn = beginTransaction();
        if (!txn) {
            return Result<void>(ErrorCode::TransactionFailed);
        }
    ...
    template<BlockHashRange R>
    Result<void> decrementBatch(R&& hashes) {
        auto txn = beginTransaction();
        if (!txn) {
            return Result<void>(ErrorCode::TransactionFailed);
        }
    ...
    Result<std::vector<std::string>> getUnreferencedBlocks(
        size_t limit = 1000,
    ...
    public:
        ...
        template<BlockHashRange R>
        void incrementBatch(R&& hashes, size_t blockSize = 0) {
            for (const auto& hash : hashes) {
...
class GarbageCollector {
public:
    GarbageCollector(ReferenceCounter& refCounter, 
    ...
    void scheduleCollection(std::chrono::seconds interval,
...
// Factory functions
std::unique_ptr<IReferenceCounter> createReferenceCounter(
