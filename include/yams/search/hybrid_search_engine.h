...
public:
    ...
    // Search interface
    virtual Result<std::vector<KeywordSearchResult>> search(
        const std::string& query,
        size_t k,
        const vector::SearchFilter* filter = nullptr) = 0;
    
    // Batch search
    virtual Result<std::vector<std::vector<KeywordSearchResult>>> batchSearch(
        const std::vector<std::string>& queries,
        size_t k,
        const vector::SearchFilter* filter = nullptr) = 0;
    
    // Index management
    virtual Result<void> addDocument(
        const std::string& id,
    ...
    virtual Result<void> updateDocument(
        const std::string& id,
        const std::string& content,
        const std::map<std::string, std::string>& metadata = {}) = 0;
    
    // Batch operations
    virtual Result<void> addDocuments(
...
public:
    ...
    Result<std::vector<HybridSearchResult>> searchWithVector(
    ...
    // Batch search
    Result<std::vector<std::vector<HybridSearchResult>>> batchSearch(
        const std::vector<std::string>& queries,
        size_t k = 10,
        const vector::SearchFilter& filter = {});
    ...
    Result<std::vector<std::string>> findSimilarTerms(
...
namespace expansion {
     ...
     */
    std::vector<std::string> expandWithEmbeddings(
        const std::string& query,
