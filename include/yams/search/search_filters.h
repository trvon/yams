...
struct DateRangeFilter {
    ...
    enum class DateField { LastModified, Created, Indexed };
    ...
    bool matches(const std::unordered_map<std::string, std::string>& metadata) const {
        ...
        if (it == metadata.end()) {
            return allowedValues
                .empty(); // Field not present - allow if no specific values required
        }
...
struct RelevanceFilter {
    ...
    bool matches(float score) const { return score >= minScore && score <= maxScore; }
};
...
public:
    ...
    void addContentTypeFilter(ContentTypeFilter filter) {
        contentTypeFilters_.push_back(std::move(filter));
    }
    ...
    void addRelevanceFilter(RelevanceFilter filter) {
        relevanceFilters_.push_back(std::move(filter));
    }
    ...
    bool hasFilters() const {
        return !dateFilters_.empty() || !sizeFilters_.empty() || !contentTypeFilters_.empty() ||
               !languageFilters_.empty() || !pathFilters_.empty() || !metadataFilters_.empty() ||
    ...
    size_t getFilterCount() const {
        return dateFilters_.size() + sizeFilters_.size() + contentTypeFilters_.size() +
               languageFilters_.size() + pathFilters_.size() + metadataFilters_.size() +
