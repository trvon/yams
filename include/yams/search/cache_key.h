...
public:
     ...
     */
    static CacheKey fromQuery(const std::string& queryText, const SearchFilters* filters = nullptr,
                              size_t offset = 0, size_t limit = 10);

    /**
     * @brief Generate cache key from parsed query
     */
    static CacheKey fromParsedQuery(const QueryNode* query, const SearchFilters* filters = nullptr,
                                    size_t offset = 0, size_t limit = 10);
