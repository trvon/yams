...
struct ValidationReport {
    ...
    double getSuccessRate() const {
        return totalChunks > 0 ? static_cast<double>(validChunks) / totalChunks : 0.0;
    }
};
...
public:
     ...
     */
    virtual ChunkValidationResult validateChunk(std::span<const std::byte> chunkData,
...
public:
     ...
     * Validate a single chunk
     */
    ChunkValidationResult validateChunk(std::span<const std::byte> chunkData,
     ...
     */
    ValidationReport
    validateManifest(const manifest::Manifest& manifest,
                     const std::function<Result<std::vector<std::byte>>(const std::string&)>&
     ...
     */
    Result<void> validateDuringReconstruction(const manifest::ChunkRef& chunkRef,
                                              std::span<const std::byte> chunkData);

    /**
     * Batch validate multiple chunks
     */
    std::vector<ChunkValidationResult>
    validateChunks(const std::vector<std::pair<std::span<const std::byte>, std::string>>& chunks);
    ...
    struct Statistics {
        ...
        double getSuccessRate() const {
            return totalValidations > 0
                       ? static_cast<double>(successfulValidations) / totalValidations
                       : 0.0;
        }
    };
...
private:
    ...
    ChunkValidationResult validateChunkInternal(std::span<const std::byte> chunkData,
                                                const std::string& expectedHash, size_t offset,
                                                size_t size);
};
