...
struct MetadataValue {
    ...
    explicit MetadataValue(const std::string& str) 
        : value(str), type(MetadataValueType::String) {}
    
    explicit MetadataValue(const char* str) 
        : value(str), type(MetadataValueType::String) {}
    ...
    explicit MetadataValue(bool b) 
        : value(b ? "1" : "0"), type(MetadataValueType::Boolean) {}
...
struct DocumentInfo {
    ...
    [[nodiscard]] int64_t indexedTimeUnix() const {
        return std::chrono::duration_cast<std::chrono::seconds>(
            indexedTime.time_since_epoch()).count();
    }
...
struct DocumentRelationship {
    ...
    [[nodiscard]] std::string getRelationshipTypeString() const {
        switch (relationshipType) {
            ...
            case RelationshipType::VersionOf: return "version_of";
            case RelationshipType::SimilarTo: return "similar_to";
            case RelationshipType::DerivedFrom: return "derived_from";
            case RelationshipType::Custom: return customType;
        }
        return "unknown";
    }
...
struct SearchHistoryEntry {
    ...
    [[nodiscard]] int64_t queryTimeUnix() const {
        return std::chrono::duration_cast<std::chrono::seconds>(
            queryTime.time_since_epoch()).count();
    }
...
struct SavedQuery {
    ...
    [[nodiscard]] int64_t lastUsedUnix() const {
        return std::chrono::duration_cast<std::chrono::seconds>(
            lastUsed.time_since_epoch()).count();
    }
...
struct DocumentMetadata {
     ...
     */
    void addMetadata(const std::string& key, const MetadataValue& value) {
        metadata[key] = value;
    }
     ...
     */
    [[nodiscard]] std::vector<DocumentRelationship> getRelationships(
...
namespace ExtractionStatusUtils {
    ...
    [[nodiscard]] inline std::string toString(ExtractionStatus status) {
        switch (status) {
            case ExtractionStatus::Pending: return "pending";
    ...
    [[nodiscard]] inline ExtractionStatus fromString(const std::string& str) {
        if (str == "pending") return ExtractionStatus::Pending;
        if (str == "success") return ExtractionStatus::Success;
        if (str == "failed") return ExtractionStatus::Failed;
        if (str == "skipped") return ExtractionStatus::Skipped;
...
namespace MetadataValueTypeUtils {
    ...
    [[nodiscard]] inline std::string toString(MetadataValueType type) {
        switch (type) {
            case MetadataValueType::String: return "string";
            case MetadataValueType::Integer: return "integer";
            case MetadataValueType::Real: return "real";
            case MetadataValueType::Blob: return "blob";
            case MetadataValueType::Boolean: return "boolean";
        }
        return "string";
    }
    ...
    [[nodiscard]] inline MetadataValueType fromString(const std::string& str) {
        if (str == "string") return MetadataValueType::String;
        if (str == "integer") return MetadataValueType::Integer;
        if (str == "real") return MetadataValueType::Real;
        if (str == "blob") return MetadataValueType::Blob;
        if (str == "boolean") return MetadataValueType::Boolean;
