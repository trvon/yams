...
public:
    ...
    virtual Result<std::unordered_map<std::string, MetadataValue>> getAllMetadata(
    ...
    virtual Result<SearchResults> fuzzySearch(const std::string& query, 
    ...
    // Bulk operations
    virtual Result<std::vector<DocumentInfo>> findDocumentsByPath(
        const std::string& pathPattern) = 0;
    virtual Result<std::vector<DocumentInfo>> findDocumentsByExtension(
...
public:
    ...
    Result<std::unordered_map<std::string, MetadataValue>> getAllMetadata(
    ...
    Result<SearchResults> search(const std::string& query, int limit = 50, 
                               int offset = 0) override;
    
    // Fuzzy search operations
    Result<SearchResults> fuzzySearch(const std::string& query, 
                                    float minSimilarity = 0.7f,
    ...
    // Bulk operations
    Result<std::vector<DocumentInfo>> findDocumentsByPath(
        const std::string& pathPattern) override;
    Result<std::vector<DocumentInfo>> findDocumentsByExtension(
...
private:
    ...
    template<typename T>
    Result<T> executeQuery(std::function<Result<T>(Database&)> func) {
