...
public:
     ...
     */
    virtual Result<std::vector<VectorRecord>> searchSimilar(
     ...
     */
    virtual Result<std::vector<VectorRecord>> getVectorsByDocument(const std::string& document_hash) = 0;
...
std::unique_ptr<IVectorBackend> createVectorBackend(VectorBackendType type = VectorBackendType::SqliteVec);
