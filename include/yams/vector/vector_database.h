...
struct VectorRecord {
    ...
    VectorRecord(std::string chunk_id, std::string document_hash, 
                 std::vector<float> embedding, std::string content)
        : chunk_id(std::move(chunk_id))
        , document_hash(std::move(document_hash))
...
public:
    ...
    std::vector<VectorRecord> searchSimilarToDocument(
    ...
    Result<std::vector<std::string>> getStaleEmbeddings(
        const std::string& model_id,
        const std::string& model_version
    );
    Result<std::vector<VectorRecord>> getEmbeddingsByVersion(
        const std::string& model_version,
        size_t limit = 1000
    );
