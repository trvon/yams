 ...
 */
enum class PipelineStage { Queued, Extracting, Processing, Indexing, Completed };
...
public:
     ...
     */
    IndexingPipeline(std::shared_ptr<metadata::MetadataRepository> metadataRepo,
     ...
     */
    std::string queueDocument(const std::filesystem::path& path, const IndexingConfig& config = {});
     ...
     */
    std::vector<std::string> queueDocuments(const std::vector<std::filesystem::path>& paths,
