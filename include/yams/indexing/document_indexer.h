 ...
 */
enum class IndexingStatus { Pending, InProgress, Completed, Failed, Skipped };
...
public:
     ...
     */
    virtual Result<IndexingResult> indexDocument(const std::filesystem::path& path,
     ...
     */
    virtual Result<std::vector<IndexingResult>>
    indexDocuments(const std::vector<std::filesystem::path>& paths,
                   const IndexingConfig& config = {}, ProgressCallback progress = nullptr) = 0;
     ...
     */
    virtual Result<IndexingResult> updateDocument(const std::filesystem::path& path,
                                                  const IndexingConfig& config = {}) = 0;
...
public:
     ...
     */
    virtual std::vector<ContentChunk> chunkContent(const std::string& content,
     ...
     */
    virtual std::vector<std::pair<std::string, double>> extractKeyTerms(const std::string& content,
