...
- **CI Build System**: Fixed GitHub Actions test workflow to source `conanbuild.sh` (build environment) instead of `conanrun.sh` (runtime environment) before meson setup, and improved meson native file detection with better error logging. This resolves "Dependency not found" errors during CI builds.
...
- **Test Build**: Fixed duplicate `main()` symbol linker error by excluding `cli/test_tui_services.cpp` from unit test suite compilation (it's a standalone test with its own main function).
