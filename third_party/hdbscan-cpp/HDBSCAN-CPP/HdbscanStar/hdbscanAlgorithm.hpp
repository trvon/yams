...
public:
    ...
    /// </summary>
    /// <param name="distances">A vector of vectors where index [i][j] indicates the jth attribute
    /// of data point i</param> <param name="k">Each point's core distance will be it's distance to
    /// the kth nearest neighbor</param> <returns> An array of core distances</returns>
    static std::vector<double> calculateCoreDistances(std::vector<std::vector<double>> distances,
                                                      int k);
    ...
    static std::vector<double> findMembershipScore(std::vector<int> clusterids,
                                                   std::vector<double> coreDistances);

    static bool propagateTree(std::vector<cluster*>& sclusters);

    /// <summary>
    /// Produces the outlier score for each point in the data set, and returns a sorted list of
    /// outlier scores.  propagateTree() must be called before calling this method.
    /// </summary>
    /// <param name="clusters">A list of Clusters forming a cluster tree which has already been
    /// propagated</param> <param name="pointNoiseLevels">A double[] with the levels at which each
    /// point became noise</param> <param name="pointLastClusters">An int[] with the last label each
    /// point had before becoming noise</param> <param name="coreDistances">An array of core
    /// distances for each data point</param> <returns>An List of OutlierScores, sorted in
    ...
    static cluster* createNewCluster(std::set<int>& points, std::vector<int>& clusterLabels,
                                     cluster* parentCluster, int clusterLabel, double edgeWeight);

    /// <summary>
    /// Calculates the number of constraints satisfied by the new clusters and virtual children of
    /// the parents of the new clusters.
    /// </summary>
    ...
    static void calculateNumConstraintsSatisfied(std::set<int>& newClusterLabels,
                                                 std::vector<cluster*>& clusters,
                                                 std::vector<hdbscanConstraint>& constraints,
