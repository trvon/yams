...
std::vector<double>
hdbscanStar::hdbscanAlgorithm::calculateCoreDistances(std::vector<std::vector<double>> distances,
                                                      int k) {
...
undirectedGraph
hdbscanStar::hdbscanAlgorithm::constructMst(std::vector<std::vector<double>> distances,
    ...
    undirectedGraph undirectedGraphObject(length, nearestMRDNeighbors, otherVertexIndices,
                                          nearestMRDDistances);
    return undirectedGraphObject;
}

void hdbscanStar::hdbscanAlgorithm::computeHierarchyAndClusterTree(
    undirectedGraph* mst, int minClusterSize, std::vector<hdbscanConstraint> constraints,
    std::vector<std::vector<int>>& hierarchy, std::vector<double>& pointNoiseLevels,
    ...
    // mst->getNumVertices());
    clusters.push_back(
        new cluster(1, NULL, std::numeric_limits<double>::quiet_NaN(), mst->getNumVertices()));
    ...
    while (currentEdgeIndex >= 0) {
        ...
        while (affectedClusterLabels.size()) {
            ...
            while (examinedVertices.size()) {
                ...
                while (unexploredSubClusterPoints.size()) {
                    ...
                    if (!incrementedChildCount && constructingSubCluster.size() >= minClusterSize &&
                        anyEdges) {
                ...
                if (numChildClusters >= 2 && constructingSubCluster.size() >= minClusterSize &&
                    anyEdges) {
            ...
            if (numChildClusters >= 2 &&
                currentClusterLabels[*firstChildCluster.begin()] == examinedClusterLabel) {
        ...
        for (std::vector<cluster*>::iterator it = newClusters.begin(); it != newClusters.end();
             it++) {
...
std::vector<int> hdbscanStar::hdbscanAlgorithm::findProminentClusters(
    std::vector<cluster*>& clusters, std::vector<std::vector<int>>& hierarchy, int numPoints) {
...
std::vector<double>
hdbscanStar::hdbscanAlgorithm::findMembershipScore(std::vector<int> clusterids,
...
/// </summary>
/// <param name="clusters">A list of Clusters forming a cluster tree which has already been
/// propagated</param> <param name="pointNoiseLevels">A double[] with the levels at which each point
/// became noise</param> <param name="pointLastClusters">An int[] with the last label each point had
/// before becoming noise</param> <param name="coreDistances">An array of core distances for each
/// data point</param> <returns>An List of OutlierScores, sorted in descending order</returns>
...
/// <param name="edgeWeight">The edge weight at which to remove the points from their previous
/// Cluster</param> <returns>The new Cluster, or null if the clusterId was 0</returns>
cluster* hdbscanStar::hdbscanAlgorithm::createNewCluster(std::set<int>& points,
                                                         std::vector<int>& clusterLabels,
                                                         cluster* parentCluster, int clusterLabel,
                                                         double edgeWeight) {
...
void hdbscanStar::hdbscanAlgorithm::calculateNumConstraintsSatisfied(
    ...
    std::vector<hdbscanConstraint>& constraints, std::vector<int>& clusterLabels) {
    ...
    for (hdbscanConstraint constraint : constraints) {
        ...
        if (constraint.getConstraintType() == hdbscanConstraintType::mustLink && labelA == labelB) {
            ...
            if (labelA != 0 && find(newClusterLabels.begin(), newClusterLabels.end(), labelA) !=
                                   newClusterLabels.end())
                clusters[labelA]->addConstraintsSatisfied(1);
            if (labelB != 0 && (find(newClusterLabels.begin(), newClusterLabels.end(), labelA) !=
                                newClusterLabels.end()))
