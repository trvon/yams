...
struct VerificationScheduler::Impl {
    ...
    explicit Impl(SchedulingStrategy strategy) : strategy(strategy) {}
    ...
    std::optional<BlockInfo> getNextBlockFromQueue() {
        ...
        spdlog::debug("Retrieved block {} from verification queue", block.hash.substr(0, 8));
    ...
    void updateBlockInfoInMap(const std::string& hash, const VerificationResult& result) {
        ...
        if (it != blockInfoMap.end()) {
            ...
            if (!result.isSuccess()) {
                block.failureCount++;
                spdlog::debug("Incremented failure count for block {} to {}", hash.substr(0, 8),
                              block.failureCount);
            } else {
            ...
            if (strategy == SchedulingStrategy::ByFailures && !result.isSuccess()) {
                verificationQueue.push(block);
                spdlog::debug("Re-queued failed block {} with higher priority", hash.substr(0, 8));
            }
        }
    }
...
void VerificationScheduler::updateBlockInfo(const std::string& hash,
