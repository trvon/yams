...
public:
    ...
    double getRate() const {
        ...
        auto actualWindow = std::chrono::duration_cast<std::chrono::milliseconds>(now - firstEventTime);
...
struct VerificationMonitor::Impl {
    ...
    void recordRepairAttemptInternal(const std::string& hash, bool success) {
        ...
        spdlog::info("Recorded repair attempt for block {}: {}", 
                    hash.substr(0, 8), success ? "SUCCESS" : "FAILED");
    }
    ...
    bool shouldAlertInternal() const {
        ...
        double errorRatio = currentVerificationRate > 0 ? 
            currentErrorRate / currentVerificationRate : 0.0;
        
        bool shouldAlert = errorRatio > errorRateThreshold;
        
        if (shouldAlert) {
            spdlog::warn("Error rate threshold exceeded: {:.2f}% > {:.2f}%", 
                        errorRatio * 100, errorRateThreshold * 100);
        }
    ...
    Metrics getCurrentMetricsInternal() const {
        ...
        if (metrics.verificationsPerSecond > 0) {
            metrics.currentErrorRate = static_cast<double>(metrics.errorsPerSecond) / 
                                     metrics.verificationsPerSecond;
        } else {
    ...
    void exportMetricsInternal(std::ostream& out) const {
        ...
        if (metrics.totalVerifications > 0) {
            double overallErrorRate = static_cast<double>(metrics.totalErrors) / metrics.totalVerifications;
            out << "kronos_verification_overall_error_rate " << std::fixed << std::setprecision(4) 
                << overallErrorRate << "\n";
        }
        
        if (metrics.repairAttempts > 0) {
            double repairSuccessRate = static_cast<double>(metrics.successfulRepairs) / metrics.repairAttempts;
        ...
        // Status breakdown
        {
            ...
            for (const auto& result : recentResults) {
                switch (result.status) {
                    case VerificationStatus::Passed: passed++; break;
                    case VerificationStatus::Failed: failed++; break;
                    case VerificationStatus::Missing: missing++; break;
                    case VerificationStatus::Corrupted: corrupted++; break;
                    case VerificationStatus::Repaired: repaired++; break;
                }
            }
...
VerificationMonitor::VerificationMonitor()
