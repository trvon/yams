...
ChunkValidationResult ChunkValidator::validateChunk(
...
ValidationReport ChunkValidator::validateManifest(
    ...
    spdlog::info("Starting validation of {} chunks for file {}", 
                 manifest.chunks.size(), manifest.fileHash.substr(0, 8));
    ...
    for (const auto& chunkRef : manifest.chunks) {
        ...
        if (!chunkResult.has_value()) {
            ChunkValidationResult result{
                ...
                .isValid = false,
                .errorMessage = "Failed to retrieve chunk: " + chunkResult.error().message,
        ...
        // Validate chunk hash
        auto validationResult = validateChunkInternal(
            chunkData, chunkRef.hash, chunkRef.offset, chunkRef.size);
    ...
    spdlog::info("Manifest validation completed: {}/{} chunks valid, took {}ms",
                report.validChunks, report.totalChunks, report.totalTime.count());
    
    return report;
}

Result<void> ChunkValidator::validateDuringReconstruction(
    ...
    auto result = validateChunkInternal(
        chunkData, chunkRef.hash, chunkRef.offset, chunkRef.size);
...
ChunkValidationResult ChunkValidator::validateChunkInternal(
    std::span<const std::byte> chunkData,
    const std::string& expectedHash,
    size_t offset,
    size_t size) {
    ...
    auto endTime = std::chrono::high_resolution_clock::now();
    result.validationTime = std::chrono::duration_cast<std::chrono::milliseconds>(
        endTime - startTime);
