...
public:
    ...
    Result<FileSignature> detectWithLibMagic(std::span<const std::byte> data) {
        ...
        const char* mimeType = magic_buffer(magicCookie, 
                                           data.data(), 
                                           data.size());
        if (!mimeType) {
    ...
    if (config.useBuiltinPatterns) {
        ...
        for (const auto& pattern : pImpl->patterns) {
            if (!pattern.mimeType.empty()) {
                ...
                bool isBinary = true;
                if (pattern.fileType == "text" || 
                    pattern.mimeType.find("text/") == 0 ||
    ...
    for (const auto& [ext, mime] : EXTENSION_MIME_MAP) {
        if (pImpl->mimeToFileType.find(mime) == pImpl->mimeToFileType.end()) {
            ...
            if (mime.find("text/") == 0) fileType = "text";
                     ...
                     mime.find("application/vnd.") == 0) fileType = "document";
...
Result<FileSignature> FileTypeDetector::detectFromFile(const std::filesystem::path& path) {
    ...
    if (!result || result.value().confidence < 0.5f) {
        ...
        if (mimeType != "application/octet-stream") {
            ...
            if (mimeType.find("image/") == 0) sig.fileType = "image";
            else if (mimeType.find("video/") == 0) sig.fileType = "video";
            else if (mimeType.find("audio/") == 0) sig.fileType = "audio";
            else if (mimeType.find("text/") == 0) sig.fileType = "text";
            else sig.fileType = "unknown";
...
Result<size_t> FileTypeDetector::loadPatternsFromDatabase(metadata::Database& db) {
    auto stmtResult = db.prepare(
        "SELECT pattern_hex, offset, file_type, mime_type, description, confidence "
...
Result<size_t> FileTypeDetector::savePatternsToDatabase(metadata::Database& db) {
    ...
    auto transactionResult = db.transaction([this, &db, &count]() -> Result<void> {
        ...
        if (!clearResult) return clearResult.error();
        
        // Insert new patterns
        auto stmtResult = db.prepare(
            "INSERT INTO file_patterns (pattern, pattern_hex, offset, file_type, "
        ...
        for (const auto& pattern : pImpl->patterns) {
            auto bindResult = stmt.bindAll(
                std::span<const std::byte>(pattern.pattern),
                pattern.patternHex,
                static_cast<int>(pattern.offset),
                pattern.fileType,
                pattern.mimeType,
                pattern.description,
                pattern.confidence
        ...
        if (!versionResult) return versionResult.error();
...
bool FileTypeDetector::isTextMimeType(const std::string& mimeType) const {
    ...
    if (mimeType.find("text/") == 0) return true;
...
std::string FileTypeDetector::getFileTypeCategory(const std::string& mimeType) const {
    ...
    if (mimeType.find("video/") == 0) return "video";
    if (mimeType.find("audio/") == 0) return "audio";
    if (mimeType.find("application/pdf") == 0 ||
        mimeType.find("application/msword") == 0 ||
        mimeType.find("application/vnd.") == 0) return "document";
    if (mimeType.find("zip") != std::string::npos ||
        mimeType.find("tar") != std::string::npos ||
...
std::vector<FilePattern> getDefaultPatterns() {
    ...
    static const PatternDef patternDefs[] = {
        ...
        {"52494646", 0, "image", "image/webp", "WebP image", 0.8f}, // RIFF header, needs more checking
        
        // Documents
        {"255044462D", 0, "document", "application/pdf", "PDF document", 1.0f},
        {"D0CF11E0A1B11AE1", 0, "document", "application/vnd.ms-office", "Microsoft Office document", 1.0f},
...
bool isBinaryData(std::span<const std::byte> data) {
    ...
    if (controlChars > checkLength / 10) return true;
    
    // If printable characters are less than 70%, likely binary
    if (printableChars < checkLength * 0.7) return true;
