...
public:
    ...
    void checkAlgorithmHealth(CompressionAlgorithm algo) {
        ...
        if (ratio > 0 && ratio < config_.compressionRatioThreshold) {
            triggerAlert(Alert{
                .type = AlertType::LowCompressionRatio,
                .algorithm = algo,
                .message = fmt::format("Low compression ratio for {}", 
                                     algorithmName(algo)),
                .value = ratio,
        ...
        // Check error rate
        uint64_t totalOps = algoStats.filesCompressed.load() + 
                           algoStats.filesDecompressed.load();
        uint64_t totalErrors = algoStats.compressionErrors.load() + 
                              algoStats.decompressionErrors.load();
        ...
        if (totalOps > 0) {
            ...
            if (errorRate > config_.errorRateThreshold) {
                triggerAlert(Alert{
                    ...
                    .algorithm = algo,
                    .message = fmt::format("High error rate for {}", 
                                         algorithmName(algo)),
                    .value = errorRate,
        ...
        if (throughput > 0 && throughput < config_.performanceThreshold) {
            triggerAlert(Alert{
                ...
                .algorithm = algo,
                .message = fmt::format("Slow compression performance for {}", 
                                     algorithmName(algo)),
                .value = throughput,
    ...
    CompressionStats getCurrentStats() const {
        return getCurrentStatsInternal();
    }
...
private:
    ...
    void monitorLoop() {
        while (running_.load()) {
            ...
            cv_.wait_for(lock, config_.metricsInterval,
    ...
    static std::string formatAlert(const Alert& alert) {
        return fmt::format("[{}] {} - Value: {:.2f}, Threshold: {:.2f}",
    ...
    static std::string alertTypeName(AlertType type) {
        switch (type) {
            case AlertType::LowCompressionRatio: return "LOW_RATIO";
            case AlertType::HighErrorRate: return "HIGH_ERROR";
            case AlertType::SlowPerformance: return "SLOW_PERF";
            case AlertType::UnusualDataPattern: return "UNUSUAL_DATA";
            case AlertType::ResourceExhaustion: return "RESOURCE_LIMIT";
            case AlertType::CompressionFailure: return "FAILURE";
...
std::vector<MetricsSnapshot> CompressionMonitor::getHistory(
...
void CompressionMonitor::setMetricCollector(
