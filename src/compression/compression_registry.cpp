...
void CompressionRegistry::registerCompressor(CompressionAlgorithm algorithm,
    ...
    spdlog::debug("Registered compressor for algorithm: {}", static_cast<int>(algorithm));
}

std::unique_ptr<ICompressor>
CompressionRegistry::createCompressor(CompressionAlgorithm algorithm) const {
    std::lock_guard lock(mutex_);

    auto it = factories_.find(algorithm);
    if (it == factories_.end()) {
        spdlog::error("Compressor not found for algorithm: {}", static_cast<int>(algorithm));
        return nullptr;
    }
...
struct NoneRegistrar {
    NoneRegistrar() {
        CompressionRegistry::instance().registerCompressor(CompressionAlgorithm::None, []() {
            // Create a passthrough compressor for "None"
            class PassthroughCompressor : public ICompressor {
            public:
                Result<CompressionResult> compress(std::span<const std::byte> data,
                                                   uint8_t) override {
                ...
                std::pair<uint8_t, uint8_t> supportedLevels() const override { return {0, 0}; }

                size_t maxCompressedSize(size_t inputSize) const override { return inputSize; }
