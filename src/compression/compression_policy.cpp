...
CompressionPolicy::CompressionPolicy(Rules rules) : rules_(std::move(rules)) {
    spdlog::debug("CompressionPolicy initialized with custom rules");
}

CompressionDecision CompressionPolicy::shouldCompress(const api::ContentMetadata& metadata,
    ...
    if (isAlreadyCompressed(metadata)) {
        return CompressionDecision::dontCompress("Content appears to be already compressed");
    }
    ...
    if (!isCompressibleType(metadata.mimeType, metadata.name)) {
        return CompressionDecision::dontCompress("File type is not compressible");
    }
...
uint8_t CompressionPolicy::selectLevel(CompressionAlgorithm algo,
...
bool CompressionPolicy::isCompressibleType(const std::string& mimeType,
    ...
    return lowerMime.find("text/") == 0 || lowerMime.find("application/") == 0;
}
...
bool CompressionPolicy::isAlreadyCompressed(const api::ContentMetadata& metadata) const {
    // Check MIME type
    auto lowerMime = toLowerCase(metadata.mimeType);
    if (lowerMime.find("compressed") != std::string::npos ||
        lowerMime.find("zip") != std::string::npos || lowerMime.find("gzip") != std::string::npos) {
        return true;
    }
...
CompressionPolicy::FileTemperature
CompressionPolicy::classifyTemperature(const AccessPattern& pattern) const {
...
public:
    ...
    [[nodiscard]] Result<void> start() {
        ...
        if (!running_.compare_exchange_strong(expected, true)) {
            return Error{ErrorCode::OperationInProgress, "Scheduler is already running"};
        }
...
private:
    ...
    void workerLoop() {
        while (running_) {
            ...
            cv_.wait_for(lock, config_.scanInterval, [this] { return !running_.load(); });
    ...
    void processBatch(const std::vector<std::string>& candidates, size_t start, size_t end) {
