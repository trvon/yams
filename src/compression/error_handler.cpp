...
namespace {
    ...
    const char* strategyToString(RecoveryStrategy strategy) noexcept {
        switch (strategy) {
            ...
            case RecoveryStrategy::Uncompressed: return "Uncompressed";
            case RecoveryStrategy::Alternative: return "Alternative";
            default: return "Unknown";
        }
    }
    ...
    const char* algorithmToString(CompressionAlgorithm algorithm) noexcept {
        switch (algorithm) {
            ...
            case CompressionAlgorithm::Zstandard: return "Zstandard";
...
CompressionError CompressionError::create(
    ErrorCode code,
...
std::string CompressionError::format() const {
    return fmt::format(
        "[{}] {} in {} ({}): {} | Algorithm: {}, Size: {}, Level: {}, Attempt: {}",
...
bool CompressionError::isRecoverable() const noexcept {
    return recommendedStrategy != RecoveryStrategy::None &&
...
public:
    ...
    RecoveryResult handleDecompressionError(
        const CompressionError& error,
    ...
    void setDegradedMode(bool degraded) {
        ...
        if (degradedMode_ != degraded) {
            ...
            spdlog::warn("Compression system degraded mode: {}", 
...
private:
    ...
    RecoveryResult executeRetryStrategy(
        ...
        for (size_t attempt = 1; attempt <= cfg.maxRetryAttempts; ++attempt) {
            ...
            spdlog::debug("Retrying compression operation, attempt {}/{}", 
                         attempt, cfg.maxRetryAttempts);
            ...
            try {
                ...
                if (retryResult.has_value()) {
                    ...
                    spdlog::debug("Retry attempt {} failed: {}", 
                                 attempt, retryResult.error().message);
                }
            } catch (const std::exception& ex) {
    ...
    RecoveryResult executeFallbackStrategy(
        const CompressionError& error,
...
CompressionErrorHandler& CompressionErrorHandler::operator=(CompressionErrorHandler&&) noexcept = default;

RecoveryResult CompressionErrorHandler::handleError(
    const CompressionError& error,
    std::function<Result<CompressionResult>()> retryFunction) {
...
CompressionError CompressionErrorHandler::fromException(
...
CompressionErrorScope::CompressionErrorScope(
    ...
    if (std::uncaught_exceptions() > 0) {
        try {
            auto error = CompressionError::create(
                ...
                operation_,
                "Uncaught exception in compression operation"
            );
            error.algorithm = algorithm_;
            
            // Log the error but don't attempt recovery in destructor
            spdlog::critical("Uncaught exception in compression operation: {}", 
                           error.format());
        } catch (...) {
...
CompressionError CompressionErrorScope::createError(
    ErrorCode code,
    const std::string& details) const {
    
    auto error = CompressionError::create(
        code,
        CompressionErrorHandler::mapErrorCodeToSeverity(code),
        operation_,
        details
    );
