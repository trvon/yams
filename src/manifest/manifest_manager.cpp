...
struct ManifestManager::Impl {
    ...
    void addToCache(const std::string& key, const Manifest& manifest) const {
        ...
        if (cache.size() >= config.cacheSize) {
            auto oldest = std::ranges::min_element(cache, 
    ...
    std::optional<Manifest> getFromCache(const std::string& key) const {
        if (!config.enableCaching) return std::nullopt;
...
Result<std::vector<std::byte>> ManifestManager::serialize(const Manifest& manifest) const {
    ...
    try {
        ...
        pb_manifest.set_modified_at(
            std::chrono::duration_cast<std::chrono::seconds>(
                manifest.modifiedAt.time_since_epoch()).count());
        ...
        spdlog::debug("Serialized manifest with {} chunks in {} ms", 
...
Result<Manifest> ManifestManager::deserialize(std::span<const std::byte> data) const {
    ...
    try {
        ...
        // Parse protobuf
        yams::manifest::FileManifest pb_manifest;
        std::string str(reinterpret_cast<const char*>(processedData.data()), 
                       processedData.size());
        ...
        for (const auto& pb_chunk : pb_manifest.chunks()) {
            manifest.chunks.emplace_back(ChunkRef{
                .hash = pb_chunk.hash(),
        ...
        if (processedData.size() < 16) { // Minimum size for magic + version + hash length + file size
            return Result<Manifest>(ErrorCode::CorruptedData);
        }
        ...
        for (uint32_t i = 0; i < numChunks; ++i) {
            ...
            // Read chunk data
            ChunkRef chunk;
            chunk.hash = std::string(reinterpret_cast<const char*>(processedData.data() + offset), chunkHashLen);
            offset += chunkHashLen;
...
Result<Manifest> ManifestManager::createManifest(
...
Result<bool> ManifestManager::validateManifest(const Manifest& manifest) const {
    try {
        ...
        if (manifest.version > Manifest::CURRENT_VERSION) {
            spdlog::warn("Manifest version {} is newer than supported version {}", 
        ...
        for (const auto& chunk : manifest.chunks) {
            if (chunk.offset != expectedOffset) {
                spdlog::error("Invalid chunk offset: expected {}, got {}", 
        ...
        if (expectedOffset != manifest.fileSize) {
            spdlog::error("File size mismatch: expected {}, calculated {}", 
                         manifest.fileSize, expectedOffset);
            return Result<bool>(false);
        }
        ...
        if (config_.enableChecksums && manifest.checksum != 0) {
            ...
            if (calculatedChecksum != manifest.checksum) {
                spdlog::error("Checksum mismatch: expected {}, calculated {}", 
                             manifest.checksum, calculatedChecksum);
                return Result<bool>(false);
            }
        }
...
Result<bool> ManifestManager::verifyFileIntegrity(
    ...
    try {
        ...
        if (actualSize != manifest.fileSize) {
            spdlog::error("File size mismatch: expected {}, actual {}", 
                         manifest.fileSize, actualSize);
            return Result<bool>(false);
        }
        ...
        if (calculatedHash != manifest.fileHash) {
            spdlog::error("File hash mismatch: expected {}, calculated {}", 
                         manifest.fileHash, calculatedHash);
            return Result<bool>(false);
        }
...
Result<void> ManifestManager::reconstructFile(
    const Manifest& manifest,
    const std::filesystem::path& outputPath,
    IChunkProvider& provider) const {
    
    spdlog::debug("Reconstructing file {} to {}", 
                 manifest.fileHash.substr(0, 8), outputPath.string());
    ...
    if (!file) {
        return Result<void>(Error{
            ...
            "Failed to create output file: " + outputPath.string()
        });
    }
    ...
    spdlog::debug("Successfully reconstructed file {} ({} bytes)", 
                 manifest.fileHash.substr(0, 8), totalWritten);
    
    return Result<void>();
}

std::future<Result<void>> ManifestManager::reconstructFileAsync(
    const Manifest& manifest,
    const std::filesystem::path& outputPath,
    IChunkProvider& provider) const {
    
    return std::async(std::launch::async, 
                     [this, manifest, outputPath, &provider]() {
...
Result<std::vector<std::byte>> ManifestManager::compressData(std::span<const std::byte> data) const {
...
Result<std::vector<std::byte>> ManifestManager::decompressData(std::span<const std::byte> data) const {
    // Placeholder - in real implementation would use zstd/lz4
    // For now, return original data
    return Result<std::vector<std::byte>>(std::vector<std::byte>(data.begin(), data.end()));
}
