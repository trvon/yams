...
PooledConnection::PooledConnection(std::unique_ptr<Database> db,
    ...
    : db_(std::move(db)), returnFunc_(returnFunc), 
...
ConnectionPool::ConnectionPool(const std::string& dbPath, 
...
Result<std::unique_ptr<PooledConnection>> ConnectionPool::acquire(
    ...
    if (!isConnectionValid(**conn)) {
        ...
        conn = std::make_unique<PooledConnection>(
            std::move(connResult).value(),
            [this](PooledConnection* c) { returnConnection(c); }
        );
    }
...
void ConnectionPool::pruneIdleConnections() {
    ...
    while (!available_.empty() && 
           keep.size() + activeConnections_ < config_.minConnections) {
