...
public:
    explicit Impl(const ModelCacheConfig& config)
        : config_(config), memory_manager_(config.max_memory_bytes) {}
    ...
    Result<void> initialize(std::shared_ptr<ModelLoader> loader) {
        ...
        if (config_.enable_preloading && !config_.preload_models.empty()) {
            for (const auto& model_id : config_.preload_models) {
                ...
                if (!result.has_value()) {
                    spdlog::warn("Failed to preload model {}: {}", model_id,
    ...
    Result<void> warmupModel(const std::string& model_id) {
        ...
        auto samples = warming::generateWarmupSamples(config_.warmup_iterations, 100);
    ...
    size_t getMemoryUsage() const { return memory_manager_.getUsedMemory(); }

    size_t getAvailableMemory() const { return memory_manager_.getAvailableMemory(); }
...
private:
    Result<std::shared_ptr<CachedModel>> loadModelInternal(const std::string& model_id) {
        ...
        stats_.avg_load_time = std::chrono::milliseconds(
            (stats_.avg_load_time.count() * (stats_.total_models_loaded - 1) + load_time.count()) /
            stats_.total_models_loaded);
...
ModelCache::ModelCache(const ModelCacheConfig& config) : pImpl(std::make_unique<Impl>(config)) {}
...
public:
    explicit Impl(size_t max_memory_bytes)
        : max_memory_(max_memory_bytes), used_memory_(0), high_watermark_(0.9) {}
    ...
    Result<void> allocate(const std::string& model_id, size_t bytes) {
        ...
        if (used_memory_ + bytes > max_memory_) {
            return Error{ErrorCode::ResourceExhausted, "Not enough memory to allocate model"};
        }
    ...
    size_t getTotalMemory() const { return max_memory_; }
