...
Result<void> SqliteVecBackend::initialize(const std::string& db_path) {
    ...
    auto result = loadSqliteVecExtension();
    if (!result) {
        close();
        return Error{ErrorCode::DatabaseError, "Failed to load sqlite-vec extension: " + result.error().message};
    }
...
Result<void> SqliteVecBackend::createTables(size_t embedding_dim) {
    ...
    std::string vector_sql = "CREATE VIRTUAL TABLE IF NOT EXISTS doc_embeddings USING vec0(embedding float[" + 
    ...
    if (!result) {
        ...
        return Error{ErrorCode::DatabaseError, "Failed to create vector table: " + result.error().message};
    }
    ...
    if (!result) {
        ...
        return Error{ErrorCode::DatabaseError, "Failed to create metadata table: " + result.error().message};
    }
    ...
    auto index_result = executeSQL("CREATE INDEX IF NOT EXISTS idx_doc_metadata_hash ON doc_metadata(document_hash)");
    if (!index_result) {
        spdlog::warn("Failed to create document_hash index: {}", index_result.error().message);
    }
    
    index_result = executeSQL("CREATE INDEX IF NOT EXISTS idx_doc_metadata_chunk_id ON doc_metadata(chunk_id)");
    if (!index_result) {
...
bool SqliteVecBackend::tablesExist() const {
    ...
    const char* vector_check_sql = "SELECT name FROM sqlite_master WHERE name='doc_embeddings' LIMIT 1";
    sqlite3_stmt* stmt;
    ...
    if (sqlite3_prepare_v2(db_, vector_check_sql, -1, &stmt, nullptr) != SQLITE_OK) {
        spdlog::debug("Failed to prepare sqlite_master check for vector table: {}", sqlite3_errmsg(db_));
        return false;
    }
    ...
    const char* metadata_check_sql = "SELECT name FROM sqlite_master WHERE name='doc_metadata' LIMIT 1";
    ...
    if (sqlite3_prepare_v2(db_, metadata_check_sql, -1, &stmt, nullptr) != SQLITE_OK) {
        spdlog::debug("Failed to prepare sqlite_master check for metadata table: {}", sqlite3_errmsg(db_));
        return false;
    }
...
Result<void> SqliteVecBackend::insertVectorInternal(const VectorRecord& record, bool manage_transaction) {
    spdlog::info("insertVectorInternal called for chunk_id: {}, model_id: {}", record.chunk_id, record.model_id);
    ...
    if (sqlite3_prepare_v2(db_, check_sql, -1, &check_stmt, nullptr) == SQLITE_OK) {
        ...
        if (sqlite3_step(check_stmt) == SQLITE_ROW) {
            ...
            spdlog::debug("Chunk {} already exists with rowid {}, will update", record.chunk_id, existing_rowid);
        }
    ...
    if (manage_transaction) {
        ...
        if (!tx_result) {
            return Error{ErrorCode::DatabaseError, "Failed to begin transaction: " + tx_result.error().message};
        }
    }
    ...
    try {
        ...
        if (existing_rowid != -1) {
            ...
            const char* update_vector_sql = "UPDATE doc_embeddings SET embedding = ? WHERE rowid = ?";
            sqlite3_stmt* update_stmt;
            
            if (sqlite3_prepare_v2(db_, update_vector_sql, -1, &update_stmt, nullptr) != SQLITE_OK) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to prepare vector update: " + getLastError()};
            }
            ...
            if (sqlite3_prepare_v2(db_, update_metadata_sql, -1, &metadata_stmt, nullptr) != SQLITE_OK) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to prepare metadata update: " + getLastError()};
            }
            ...
            const char* vector_sql = "INSERT INTO doc_embeddings (rowid, embedding) VALUES (NULL, ?)";
            sqlite3_stmt* vector_stmt;
            
            spdlog::info("Preparing vector insert SQL: {}", vector_sql);
            spdlog::info("Vector JSON: {}", vec_json.str().substr(0, 100) + "...");  // Log first 100 chars
            
            if (sqlite3_prepare_v2(db_, vector_sql, -1, &vector_stmt, nullptr) != SQLITE_OK) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to prepare vector insert: " + getLastError()};
            }
            ...
            if (rc != SQLITE_DONE) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to insert vector: " + getLastError()};
            }
            ...
            if (sqlite3_prepare_v2(db_, metadata_sql, -1, &metadata_stmt, nullptr) != SQLITE_OK) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to prepare metadata insert: " + getLastError()};
            }
            
            sqlite3_bind_int64(metadata_stmt, 1, vector_rowid);
            sqlite3_bind_text(metadata_stmt, 2, record.document_hash.c_str(), -1, SQLITE_TRANSIENT);
            ...
            if (rc != SQLITE_DONE) {
                if (manage_transaction) executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to insert metadata: " + getLastError()};
            }
        }
        ...
        if (manage_transaction) {
            ...
            if (!commit_result) {
                executeSQL("ROLLBACK");
                return Error{ErrorCode::DatabaseError, "Failed to commit transaction: " + commit_result.error().message};
            }
        }
...
Result<void> SqliteVecBackend::updateVector(const std::string& chunk_id, const VectorRecord& record) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
        ...
        if (sqlite3_prepare_v2(db_, get_rowid_sql, -1, &rowid_stmt, nullptr) != SQLITE_OK) {
            executeSQL("ROLLBACK");
            return Error{ErrorCode::DatabaseError, "Failed to prepare rowid query: " + getLastError()};
        }
        ...
        // Update the metadata
        const char* metadata_sql = "UPDATE doc_metadata SET document_hash = ?, chunk_text = ?, model_id = ?, metadata = ? WHERE rowid = ?";
        sqlite3_stmt* metadata_stmt;
...
Result<void> SqliteVecBackend::deleteVector(const std::string& chunk_id) {
    ...
    try {
        ...
        if (sqlite3_prepare_v2(db_, metadata_sql, -1, &metadata_stmt, nullptr) != SQLITE_OK) {
            executeSQL("ROLLBACK");
            return Error{ErrorCode::DatabaseError, "Failed to prepare metadata delete: " + getLastError()};
        }
        ...
        if (sqlite3_prepare_v2(db_, vector_sql, -1, &vector_stmt, nullptr) != SQLITE_OK) {
            executeSQL("ROLLBACK");
            return Error{ErrorCode::DatabaseError, "Failed to prepare vector delete: " + getLastError()};
        }
...
Result<void> SqliteVecBackend::deleteVectorsByDocument(const std::string& document_hash) {
    ...
    try {
        ...
        if (sqlite3_prepare_v2(db_, get_rowids_sql, -1, &rowids_stmt, nullptr) != SQLITE_OK) {
            executeSQL("ROLLBACK");
            return Error{ErrorCode::DatabaseError, "Failed to prepare rowids query: " + getLastError()};
        }
...
Result<std::vector<VectorRecord>> SqliteVecBackend::searchSimilar(
...
Result<std::optional<VectorRecord>> SqliteVecBackend::getVector(const std::string& chunk_id) {
    ...
    const char* sql = "SELECT m.document_hash, m.chunk_text, m.model_id, m.metadata, m.rowid, e.embedding "
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (metadata_json && strlen(metadata_json) > 2) {
            ...
            while (pos < json_str.length() - 1) {
                ...
                if (pos < json_str.length() && json_str[pos] == ',') pos++;
            }
        }
        // Parse embedding from joined doc_embeddings (column index 5), handling both TEXT(JSON) and BLOB
        int col_type = sqlite3_column_type(stmt, 5);
        if (col_type == SQLITE_TEXT) {
            const char* embedding_json = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));
            if (embedding_json) {
...
Result<std::vector<VectorRecord>> SqliteVecBackend::getVectorsByDocument(const std::string& document_hash) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        spdlog::info("getVectorsByDocument: Embedding column type: {}", col_type == SQLITE_TEXT ? "TEXT" : col_type == SQLITE_BLOB ? "BLOB" : "OTHER");
        
        if (col_type == SQLITE_TEXT) {
            const char* embedding_json = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4));
            if (embedding_json) {
                std::string emb_str = embedding_json;
                spdlog::info("getVectorsByDocument: Parsing embedding JSON (first 100 chars): {}", emb_str.substr(0, 100));
                std::stringstream ss(emb_str);
                char ch;
                if (ss >> ch && ch == '[') {
                    while (ss) {
    ...
    spdlog::info("getVectorsByDocument: Found {} results for document_hash: {}", results.size(), document_hash);
...
Result<void> SqliteVecBackend::loadSqliteVecExtension() {
    ...
    if (rc != SQLITE_OK) {
        ...
        return Error{ErrorCode::DatabaseError, "Failed to initialize sqlite-vec extension: " + error};
    }
    ...
    if (sqlite3_prepare_v2(db_, test_sql, -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        if (!vec0_available) {
            return Error{ErrorCode::DatabaseError, "sqlite-vec extension loaded but vec0 module not available"};
        }
    } else {
        spdlog::warn("Could not verify vec0 module availability, but extension initialization succeeded");
    }
