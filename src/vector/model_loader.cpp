...
public:
    ...
    Result<std::shared_ptr<void>> loadModel(
        ...
        spdlog::info("Loaded ONNX model from {} in {}ms", 
            path, model->info.load_time.count());
...
Result<std::shared_ptr<void>> ONNXModelLoader::loadModel(
    const std::string& path,
    const LoadOptions& options
) {
...
public:
    ...
    Result<std::shared_ptr<void>> loadModel(
        ...
        if (stats_.total_loads > 0) {
            stats_.avg_load_time = std::chrono::milliseconds(
                stats_.total_load_time.count() / stats_.total_loads
            );
        }
    ...
    Result<std::shared_ptr<void>> loadModelFromRegistry(
    ...
    std::future<Result<std::shared_ptr<void>>> loadModelAsync(
        const std::string& path,
        const LoadOptions& options
    ) {
    ...
    Result<std::string> detectFormat(const std::string& path) const {
        ...
        if (ext == "onnx") return Result<std::string>(std::string("ONNX"));
        if (ext == "pb") return Result<std::string>(std::string("TensorFlow"));
        if (ext == "pt" || ext == "pth") return Result<std::string>(std::string("PyTorch"));
        if (ext == "tflite") return Result<std::string>(std::string("TFLite"));
    ...
    ModelLoader::LoaderStats getStats() const {
        return stats_;
    }
...
Result<std::shared_ptr<void>> ModelLoader::loadModel(
    const std::string& path,
    const LoadOptions& options
) {
    return pImpl->loadModel(path, options);
...
Result<std::shared_ptr<void>> ModelLoader::loadModelFromRegistry(
    const ModelInfo& info,
    const LoadOptions& options
) {
...
std::future<Result<std::shared_ptr<void>>> ModelLoader::loadModelAsync(
    const std::string& path,
    const LoadOptions& options
) {
...
Result<void> quantizeModel(
    ...
    try {
        ...
        spdlog::info("Quantized model ({}) saved to: {}", 
            quantization_type, output_path);
    } catch (const std::exception& e) {
        return Error{ErrorCode::InternalError, 
...
Result<bool> canMigrate(
    const ModelInfo& from_model,
    const ModelInfo& to_model
) {
    ...
    if (from_model.embedding_dimension != to_model.embedding_dimension) {
        spdlog::warn("Dimension mismatch: {} -> {}", 
            from_model.embedding_dimension, 
...
Result<std::vector<float>> migrateEmbedding(
    ...
    spdlog::debug("Migrating embedding from {} to {}", 
