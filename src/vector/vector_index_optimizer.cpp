...
public:
    ...
    Result<OptimizationResult> compact() {
        ...
        try {
            ...
            for (const auto& id : all_ids.value()) {
                ...
                optimization_progress_ = 0.3 * (active_ids.size() + deleted_ids.size()) / all_ids.value().size();
            ...
            result.success = true;
            result.message = std::format("Compaction completed: removed {} vectors, merged {} segments",
    ...
    Result<OptimizationResult> rebuild() {
        ...
        try {
            ...
            result.success = true;
            result.message = std::format("Rebuild completed: {} vectors reindexed", all_vectors.size());
    ...
    OptimizationRecommendations getRecommendations() const {
        ...
        if (stats.fragmentation_ratio > 0.3) {
            ...
            recs.additional_recommendations.push_back("High fragmentation detected - compaction recommended");
        }
        
        // Check balance
        if (stats.cluster_balance_ratio < 0.7) {
            recs.should_rebalance = true;
            recs.additional_recommendations.push_back("Unbalanced index structure - rebalancing recommended");
        }
        ...
        if (stats.fragmentation_ratio > 0.7 && stats.deleted_vectors > stats.total_vectors * 0.3) {
            ...
            recs.primary_recommendation = "Complete index rebuild recommended due to severe fragmentation";
    ...
    std::vector<OptimizationResult> getOptimizationHistory(size_t limit) const {
        ...
        return std::vector<OptimizationResult>(
            optimization_history_.end() - limit,
...
private:
    ...
    void updateStatistics() {
        ...
        statistics_.fragmentation_ratio = static_cast<double>(statistics_.deleted_vectors) / statistics_.total_vectors;
    ...
    std::unique_ptr<VectorIndexManager> createOptimizedIndex(const std::vector<VectorData>& vectors) {
    ...
    void runOptimizationScheduler() {
        while (!stop_optimization_) {
            ...
            if (schedule_cv_.wait_until(lock, next_optimization, [this] {
                ...
                if (optimization_paused_) continue;
            }
...
VectorIndexOptimizer::VectorIndexOptimizer()
    : pImpl(std::make_unique<Impl>(nullptr)) {}
