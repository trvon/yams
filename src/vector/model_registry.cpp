...
public:
    ...
    Result<void> registerModel(const ModelInfo& info) {
        ...
        spdlog::info("Registered model: {} v{} ({}D)", info.name, info.version,
    ...
    Result<void> unregisterModel(const std::string& model_id) {
        ...
        dim_models.erase(std::remove(dim_models.begin(), dim_models.end(), model_id),
                         dim_models.end());

        auto& ver_models = version_index_[it->second.name];
        ver_models.erase(std::remove(ver_models.begin(), ver_models.end(), model_id),
                         ver_models.end());
    ...
    selectBestModel(size_t required_dimension,
        ...
        candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
                                        [&requirements](const ModelInfo& info) {
                                            return !ModelCompatibilityChecker::meetsRequirements(
                                                info, requirements);
                                        }),
                         candidates.end());
        ...
        std::sort(candidates.begin(), candidates.end(), [](const ModelInfo& a, const ModelInfo& b) {
    ...
    Result<void> updateMetrics(const std::string& model_id, double inference_time_ms,
        ...
        if (success) {
            ...
            metrics.min_inference_time_ms =
                std::min(metrics.min_inference_time_ms, inference_time_ms);
            metrics.max_inference_time_ms =
                std::max(metrics.max_inference_time_ms, inference_time_ms);
        } else {
...
Result<std::string>
ModelRegistry::selectBestModel(size_t required_dimension,
                               const std::map<std::string, std::string>& requirements) const {
    return pImpl->selectBestModel(required_dimension, requirements);
}

Result<void> ModelRegistry::updateMetrics(const std::string& model_id, double inference_time_ms,
                                          bool success) {
...
bool ModelCompatibilityChecker::areCompatible(const ModelInfo& model1, const ModelInfo& model2) {
    ...
    auto it =
        std::find(model1.compatible_with.begin(), model1.compatible_with.end(), model2.model_id);
    if (it != model1.compatible_with.end()) {
        return true;
    }

    // Check if one replaces the other
    it = std::find(model1.replaces.begin(), model1.replaces.end(), model2.model_id);
...
bool ModelCompatibilityChecker::meetsRequirements(
    const ModelInfo& model, const std::map<std::string, std::string>& requirements) {
