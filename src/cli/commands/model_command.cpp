...
static const std::vector<ModelInfo> AVAILABLE_MODELS = {
    {
        ...
        "Lightweight 384-dim embeddings for semantic search",
        90,
        "embedding"
    },
    {
...
private:
    Result<void> listModels() {
        ...
        for (const auto& model : AVAILABLE_MODELS) {
            std::cout << "  " << model.name << " (" << model.type << ", " 
                      << model.size_mb << " MB)\n";
    ...
    Result<void> downloadModel(const std::string& model_name, const std::string& output_dir) {
        ...
        if (system("which curl > /dev/null 2>&1") == 0) {
            download_cmd = "curl -L --progress-bar -o \"" + model_file.string() + 
                          "\" \"" + model.url + "\"";
        } else if (system("which wget > /dev/null 2>&1") == 0) {
            download_cmd = "wget --show-progress -O \"" + model_file.string() + 
                          "\" \"" + model.url + "\"";
        } else {
            return Error{ErrorCode::NotFound, "Neither curl nor wget found. Please install one to download models."};
        }
        ...
        if (result != 0) {
            ...
            return Error{ErrorCode::InternalError, "Download failed with code: " + std::to_string(result)};
        }
