...
public:
    ...
    void registerCommand(CLI::App& app, YamsCLI* cli) override {
        ...
        scopeGroup->add_option("-c,--collection", collection_, "Restore all documents from collection");
        scopeGroup->add_option("--snapshot-id", snapshotId_, "Restore all documents from snapshot ID");
        scopeGroup->add_option("--snapshot-label", snapshotLabel_, "Restore all documents from snapshot label");
        ...
        // Control options
        cmd->add_flag("--dry-run", dryRun_, "Show what would be restored without actually restoring");
        cmd->add_flag("--overwrite", overwrite_, "Overwrite existing files");
        cmd->add_flag("--create-dirs", createDirs_, "Create output directories if they don't exist");
...
private:
    Result<std::vector<metadata::DocumentInfo>> getDocumentsToRestore(
    ...
    std::vector<metadata::DocumentInfo> filterDocuments(
    ...
    Result<std::string> expandLayoutTemplate(const metadata::DocumentInfo& doc, 
        ...
        for (const auto& doc : documents) {
            ...
            if (!metadataResult) {
                spdlog::warn("Failed to get metadata for {}: {}", 
                           doc.fileName, metadataResult.error().message);
                failureCount++;
                continue;
            }
            ...
            auto layoutResult = expandLayoutTemplate(doc, metadataResult.value());
            if (!layoutResult) {
                spdlog::error("Failed to expand layout template for {}: {}",
                            doc.fileName, layoutResult.error().message);
                failureCount++;
                continue;
            }
            ...
            if (!result) {
                spdlog::error("Failed to restore {}: {}", 
                            doc.fileName, result.error().message);
                failureCount++;
                continue;
            }
