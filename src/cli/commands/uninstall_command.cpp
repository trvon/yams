...
public:
    ...
    std::string getDescription() const override {
        return "Remove YAMS from your system";
    }
    ...
    void registerCommand(CLI::App& app, YamsCLI* cli) override {
        ...
        cmd->add_flag("--purge", purge_, 
            "Remove all data, config, and cache directories");
        cmd->add_flag("--keep-config", keepConfig_, 
            "Preserve configuration files");
        cmd->add_option("--backup-to", backupPath_, 
            "Create backup before removing")
            ->check(CLI::NonexistentPath);
        cmd->add_flag("-y,--yes", skipConfirmation_, 
        ...
        cmd->callback([this]() {
            ...
            fs::path homeDir = std::getenv("HOME") ? fs::path(std::getenv("HOME")) : fs::current_path();
