...
public:
    ...
    void RenderMultiPaneLayout() {
        ...
        ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y - marginTop - marginBottom));
    ...
    void RenderCompactLayout() {
        ...
        ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y - 2 * margin));
        
        if (ImGui::Begin("Files", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove)) {
        ...
        switch (state.activeColumn) {
            ...
            case Column::Preview:
                ...
                if (state.selected >= 0 && state.selected < static_cast<int>(state.documents.size())) {
                    ImGui::SameLine();
                    ImGui::TextDisabled(" > %.20s...", state.documents[state.selected].name.c_str());
                }
                break;
        }
    ...
    void RenderRetroPreview() {
        ...
        ImVec4 borderColor = isActive ? ImVec4(1.0f, 1.0f, 0.0f, 1.0f) : ImVec4(0.0f, 1.0f, 0.0f, 1.0f);
    ...
    void RenderViewer() {
        ...
        if (ImGui::Begin("Document Viewer", &viewerOpen, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse)) {
            ...
            ImGui::BeginChild("ViewerContent", ImVec2(0, ImGui::GetContentRegionAvail().y - controlsHeight));
    ...
    void HandleLayoutAwareInput() {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_Escape)) {
            ...
            } else if (state.layoutMode == LayoutMode::SinglePane && state.activeColumn != Column::Documents) {
    ...
    void HandleMultiPaneNavigation() {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_Tab)) {
            if (io.KeyShift) {
                state.activeColumn = static_cast<Column>((static_cast<int>(state.activeColumn) + 2) % 3);
            } else {
                state.activeColumn = static_cast<Column>((static_cast<int>(state.activeColumn) + 1) % 3);
            }
        ...
        if (ImGui::IsKeyPressed('h')) {
            ...
            state.activeColumn = static_cast<Column>((static_cast<int>(state.activeColumn) + 2) % 3);
            spdlog::debug("TUI: Vim 'h' - moved to column {}", static_cast<int>(state.activeColumn));
        } else if (ImGui::IsKeyPressed('l')) {
            // Move right (next column)
            state.activeColumn = static_cast<Column>((static_cast<int>(state.activeColumn) + 1) % 3);
            spdlog::debug("TUI: Vim 'l' - moved to column {}", static_cast<int>(state.activeColumn));
        }
    ...
    void HandleVerticalNavigation() {
        ...
        // Page navigation
        if (ImGui::IsKeyPressed(ImGuiKey_PageDown) || (ImGui::IsKeyPressed('d') && ImGui::GetIO().KeyCtrl)) {
            HandlePageDown();
            navigationHandled = true;
        } else if (ImGui::IsKeyPressed(ImGuiKey_PageUp) || (ImGui::IsKeyPressed('u') && ImGui::GetIO().KeyCtrl)) {
            HandlePageUp();
            navigationHandled = true;
        }
    ...
    void HandlePageDown() {
        if (state.activeColumn == Column::Preview) {
            ...
            if (!state.documents.empty()) {
                ...
                state.selected = std::min(state.selected + pageSize, static_cast<int>(state.documents.size()) - 1);
            }
        }
    }
    ...
    void RenderVirtualDocumentList() {
        ...
        ImGui::BeginChild("DocumentList", ImVec2(0, 0), false, ImGuiWindowFlags_AlwaysVerticalScrollbar);
        ...
        for (int i = 0; i < totalItems; ++i) {
            ...
            if (ImGui::Selectable(itemLabel.c_str(), isSelected, ImGuiSelectableFlags_SpanAllColumns)) {
                state.selected = i;
                UpdatePreview();
            }
    ...
    void Run() {
        ...
        while (!shouldExit) {
            ...
            if (frameCount == 1 || frameCount % 60 == 0) {
                ...
                spdlog::debug("Frame {}, loading: {}, complete: {}, docs: {}", 
                             frameCount, isLoading.load(), loadingComplete.load(), docCount);
            }
