...
public:
    ...
    std::string getDescription() const override { return "Migrate YAMS data and configuration"; }
    ...
    void registerCommand(CLI::App& app, YamsCLI* cli) override {
        ...
        auto* storageCmd = cmd->add_subcommand("storage", "Migrate storage to a new location");
        storageCmd->add_option("destination", destination_, "New storage path")
            ->required()
            ->check(CLI::NonexistentPath);
        storageCmd->add_flag("--verify", verify_, "Verify integrity after migration");
        storageCmd->add_flag("--cleanup", cleanup_, "Remove source after successful migration");
        storageCmd->callback([this]() { showStorageMigration(); });

        // Config migration subcommand
        auto* configCmd = cmd->add_subcommand("config", "Migrate configuration to a new location");
        configCmd->add_option("destination", destination_, "New config directory")->required();
        configCmd->callback([this]() { showConfigMigration(); });

        // Export subcommand
        auto* exportCmd = cmd->add_subcommand("export", "Export all data to archive");
        exportCmd->add_option("archive", archivePath_, "Archive file path")->required();
        exportCmd->add_flag("--compress", compress_, "Compress the archive");
        exportCmd->add_flag("--encrypt", encrypt_, "Encrypt the archive");
        exportCmd->callback([this]() { showExport(); });

        // Import subcommand
        auto* importCmd = cmd->add_subcommand("import", "Import data from archive");
        importCmd->add_option("archive", archivePath_, "Archive file to import")
            ->required()
            ->check(CLI::ExistingFile);
        importCmd->add_flag("--overwrite", overwrite_, "Overwrite existing data");
        importCmd->callback([this]() { showImport(); });
    }
