...
yams::Result<std::string> ensureAuthenticatedDirectStore(const std::filesystem::path& dataDir,
    ...
    if (std::filesystem::exists(marker, error)) {
        ...
        auto encoded = service_manager_detail::readProtectedP2pPrivateKey(marker);
        if (!encoded) {
    ...
    if (auto written =
            service_manager_detail::writeExclusiveP2pPrivateKey(temporary, content.dump());
        !written) {
    ...
    try {
        ...
        auto svc = yams::memory_sync::createMemorySyncService(
            ...
            policy.transport == "direct"
                ? yams::memory_sync::
                      WriterPrivateKeyReader{service_manager_detail::readProtectedP2pPrivateKey}
                : yams::memory_sync::WriterPrivateKeyReader{},
            policy.transport == "direct"
                ? yams::memory_sync::
                      WriterTrustFileReader{service_manager_detail::readProtectedP2pTrustFile}
