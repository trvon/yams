...
EmbeddingService::EmbeddingService(std::shared_ptr<api::IContentStore> store,
    ...
    : config_(std::move(config)), effectiveConfig_(resolveConcurrencyPolicy(config_)),
      ...
      strand_(coordinator_->makeStrand()), semanticGraph_(config_.semanticGraph) {
    semanticGraph_.setEdgeSink(
        [this](std::vector<metadata::KGEdge> edges, std::string_view source) {
            auto* coord = getWriteCoordinator_ ? getWriteCoordinator_() : nullptr;
            if (!coord) {
                return false;
            }
            auto batch = std::make_unique<WriteBatch>();
            batch->source = std::string(source);
            batch->ops.emplace_back(AddEdgesOp{std::move(edges), /*unique=*/true});
            coord->enqueue(std::move(batch));
            return true;
        });
    semanticGraph_.setPhaseTimer(
        [this](std::string_view phase, std::chrono::steady_clock::time_point start) {
...
void EmbeddingService::updateSemanticNeighborGraph(
    ...
    semanticGraph_.update(kgStore, vdb, modelName, sourceDocuments, sourceAllCorpus);
}
...
EmbeddingService::rebuildSemanticNeighborGraphForCorpus(const std::string& modelName) {
    ...
    const auto before = semanticGraph_.edgesCreated();
    semanticGraph_.update(kgStore, vdb, modelName, {}, true);
    if (auto* coord = getWriteCoordinator_ ? getWriteCoordinator_() : nullptr) {
        auto flushRes = coord->flush();
        if (!flushRes) {
            return flushRes.error();
    ...
    const auto after = semanticGraph_.edgesCreated();
...
Result<std::size_t> EmbeddingService::backfillSemanticNeighborGraph(const std::string& modelName,
    ...
    semanticGraph_.update(kgStore, vdb, modelName, sources, /*sourceAllCorpus=*/false);
    return sources.size();
}
