...
REGISTER_EXTRACTOR(PlainTextExtractor,
    ...
    ".sql", ".conf", ".cfg", ".ini", ".toml",
    ".tex", ".bib", ".rst", ".adoc", ".org");

Result<ExtractionResult> PlainTextExtractor::extract(
    ...
    try {
        ...
        spdlog::debug("First 100 chars: {}", result.text.substr(0, std::min<size_t>(100, result.text.length())));
...
Result<ExtractionResult> PlainTextExtractor::extractFromBuffer(
...
Result<std::string> PlainTextExtractor::readFileWithEncoding(
    const std::filesystem::path& path,
    const ExtractionConfig& config) {
...
void PlainTextExtractor::extractFileMetadata(ExtractionResult& result,
    ...
    try {
        ...
        result.metadata["last_modified"] = std::to_string(
