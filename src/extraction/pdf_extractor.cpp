...
Result<ExtractionResult> PdfExtractor::extract(const std::filesystem::path& path, const ExtractionConfig& config) {
...
Result<ExtractionResult> PdfExtractor::extractFromBuffer(std::span<const std::byte> data, const ExtractionConfig& config) {
    PdfExtractOptions options;
    options.extractAll = true;
    options.extractMetadata = true;
...
Result<ExtractionResult> PdfExtractor::extractInternal(std::span<const std::byte> data,
    ...
    int maxPages = (options.maxPages > 0) ? 
                   std::min(options.maxPages, pageCount) : pageCount;
...
void PdfExtractor::extractMetadata(FPDF_DOCUMENT doc, ExtractionResult& result) {
    ...
    auto getMetaString = [doc](const std::string& tag) -> std::string {
        ...
        if (bufferLen <= 2) return "";  // Empty or just null terminator
    ...
    if (!subject.empty()) result.metadata["subject"] = subject;
    if (!keywords.empty()) result.metadata["keywords"] = keywords;
    if (!creator.empty()) result.metadata["creator"] = creator;
    if (!producer.empty()) result.metadata["producer"] = producer;
    if (!creationDate.empty()) result.metadata["creation_date"] = creationDate;
    if (!modDate.empty()) result.metadata["modification_date"] = modDate;
...
std::vector<std::string> PdfExtractor::findTextInContext(const std::vector<std::string>& lines,
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (lineLower.find(searchLower) != std::string::npos) {
            // Add context before
            size_t startIdx = (i >= static_cast<size_t>(contextLines)) ? 
                            i - contextLines : 0;
