...
static const std::vector<std::string> sampleWords = {
    "hello", "world", "search", "engine", "fuzzy", "matching",
    "algorithm", "benchmark", "performance", "testing", "query",
    "database", "index", "retrieval", "similarity", "distance"
};
...
static void BM_BKTree_Construction(benchmark::State& state) {
    ...
    state.counters["insert_qps"] = benchmark::Counter(
        static_cast<double>(vocab) * static_cast<double>(state.iterations()),
        benchmark::Counter::kIsRate);
}
BENCHMARK(BM_BKTree_Construction)
...
static void BM_BKTree_Search(benchmark::State& state) {
    ...
    if (state.iterations() > 0) {
        state.counters["avg_matches"] = static_cast<double>(total_matches) / static_cast<double>(state.iterations());
