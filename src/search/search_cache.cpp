...
SearchCache::SearchCache(const SearchCacheConfig& config) : config_(config) {
...
void SearchCache::put(const CacheKey& key, const SearchResults& results, std::chrono::seconds ttl) {
...
std::unique_ptr<SearchCache::CacheEntry>
SearchCache::deserializeEntry(const std::string& data) const {
...
std::unique_ptr<SearchCache> SearchCacheFactory::create(const SearchCacheConfig& config) {
