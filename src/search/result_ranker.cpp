...
float ResultRanker::calculateScore(const SearchResultItem& item, const QueryNode* query) const {
    ...
    score = tfIdfScore + 
            fieldBoostScore + 
            freshnessScore * config_.freshnessWeight +
            sizeScore * config_.sizeWeight +
            positionScore * config_.positionWeight +
            languageScore +
            phraseBonus +
            exactMatchBonus;
...
void ResultRanker::rankResults(std::vector<SearchResultItem>& results, const QueryNode* query) const {
...
float ResultRanker::calculateTFIDF(const std::string& term, float termFrequency, const std::string& field) const {
    if (termFrequency == 0.0f) return 0.0f;
...
float ResultRanker::calculateFreshnessScore(const std::chrono::system_clock::time_point& lastModified) const {
    ...
    float freshnessScore = std::exp(-static_cast<float>(age) / (24.0f * 30.0f)); // Decay over 30 days
...
float ResultRanker::calculateSizeScore(size_t fileSize) const {
    ...
    float sizeDiff = std::abs(static_cast<float>(fileSize) - static_cast<float>(config_.optimalSize));
...
float ResultRanker::calculatePositionScore(const std::vector<SearchHighlight>& highlights) const {
    if (highlights.empty()) return 0.0f;
    
    float positionScore = 0.0f;
    
    for (const auto& highlight : highlights) {
        // Earlier positions (lower offset) get higher scores
        float normalizedPosition = 1.0f / (1.0f + static_cast<float>(highlight.startOffset) / 1000.0f);
...
float ResultRanker::calculatePhraseBonus(const SearchResultItem& item, const QueryNode* query) const {
    ...
    auto queryTerms = extractQueryTerms(query);
    if (queryTerms.size() < 2) return 0.0f;
...
float ResultRanker::calculateExactMatchBonus(const SearchResultItem& item, const QueryNode* query) const {
    auto queryTerms = extractQueryTerms(query);
...
void ResultRanker::extractTermsRecursive(const QueryNode* node, std::vector<std::string>& terms) const {
    if (!node) return;
...
void ResultRanker::updateTermFrequencies(const std::string& documentText, const std::string& documentId) {
...
void AdvancedRanker::rankResults(std::vector<SearchResultItem>& results, const QueryNode* query) const {
    switch (algorithm_) {
...
float AdvancedRanker::calculateBM25Score(const SearchResultItem& item, const QueryNode* query) const {
    ...
    for (const auto& term : queryTerms) {
        ...
        float denominator = tf + k1 * (1.0f - b + b * (static_cast<float>(item.fileSize) / avgDocLength));
...
float AdvancedRanker::calculateCosineScore(const SearchResultItem& item, const QueryNode* query) const {
...
float AdvancedRanker::calculateHybridScore(const SearchResultItem& item, const QueryNode* query) const {
