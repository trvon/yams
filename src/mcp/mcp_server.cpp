...
#if defined(YAMS_WASI)
    (void)req;
    co_return Error{ErrorCode::NotSupported, "suggest_context is not supported on WASI build"};
#else
...
MCPServer::handleSemanticDedupe(const MCPSemanticDedupeRequest& req) {
#if defined(YAMS_WASI)
    (void)req;
    co_return Error{ErrorCode::NotSupported, "semantic_dedupe is not supported on WASI build"};
#else
    auto dataDir = daemon_client_config_.dataDir;
    if (dataDir.empty()) {
        dataDir = yams::config::get_data_dir();
