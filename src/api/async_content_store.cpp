...
struct AsyncContentStore::Impl {
    ...
    void waitForSlot() {
        ...
        opCv.wait(lock, [this] {
            return pendingOps < maxConcurrentOps || cancelRequested;
        });
    ...
    template<typename F>
    auto runAsync(F&& func) -> std::future<decltype(func())> {
        waitForSlot();
...
AsyncContentStore::AsyncContentStore(std::shared_ptr<IContentStore> store)
    : pImpl(std::make_unique<Impl>(std::move(store), 10))
    , store_(pImpl->store) {}
...
std::future<Result<StoreResult>> AsyncContentStore::storeAsync(
...
std::future<Result<RetrieveResult>> AsyncContentStore::retrieveAsync(
...
std::future<Result<StoreResult>> AsyncContentStore::storeStreamAsync(
    std::istream& stream,
    const ContentMetadata& metadata,
    ProgressCallback progress) {
...
std::future<Result<RetrieveResult>> AsyncContentStore::retrieveStreamAsync(
    const std::string& hash,
    std::ostream& output,
    ProgressCallback progress) {
...
std::future<Result<bool>> AsyncContentStore::existsAsync(const std::string& hash) {
    return pImpl->runAsync([store = pImpl->store, hash]() {
...
std::future<Result<ContentMetadata>> AsyncContentStore::getMetadataAsync(
    const std::string& hash) {
    
    return pImpl->runAsync([store = pImpl->store, hash]() {
        return store->getMetadata(hash);
    });
}

std::future<Result<void>> AsyncContentStore::updateMetadataAsync(
    const std::string& hash,
    const ContentMetadata& metadata) {
    
    return pImpl->runAsync([store = pImpl->store, hash, metadata]() {
        return store->updateMetadata(hash, metadata);
    });
}

// Callback-based async operations
void AsyncContentStore::storeAsync(
    const std::filesystem::path& path,
...
void AsyncContentStore::retrieveAsync(
    const std::string& hash,
    const std::filesystem::path& outputPath,
...
void AsyncContentStore::existsAsync(
    const std::string& hash,
...
void AsyncContentStore::removeAsync(
    const std::string& hash,
    BoolCallback callback) {
...
void AsyncContentStore::getMetadataAsync(
    const std::string& hash,
...
void AsyncContentStore::updateMetadataAsync(
    const std::string& hash,
    const ContentMetadata& metadata,
...
// Batch operations
std::future<std::vector<Result<StoreResult>>> AsyncContentStore::storeBatchAsync(
    const std::vector<std::filesystem::path>& paths,
    const std::vector<ContentMetadata>& metadata) {
    
    return pImpl->runAsync([store = pImpl->store, paths, metadata]() {
        return store->storeBatch(paths, metadata);
    });
}

std::future<std::vector<Result<bool>>> AsyncContentStore::removeBatchAsync(
    const std::vector<std::string>& hashes) {
    
    return pImpl->runAsync([store = pImpl->store, hashes]() {
        return store->removeBatch(hashes);
    });
}

// Maintenance operations
std::future<Result<void>> AsyncContentStore::verifyAsync(ProgressCallback progress) {
    return pImpl->runAsync([store = pImpl->store, progress]() {
...
void AsyncContentStore::waitAll() {
    ...
    pImpl->opCv.wait(lock, [this] {
        return pImpl->pendingOps == 0;
    });
}
