...
std::future<CreateMetadataResponse> MetadataApi::createMetadataAsync(const CreateMetadataRequest& request) {
...
BulkCreateResponse MetadataApi::bulkCreate(const BulkCreateRequest& request) {
    ...
    try {
        ...
        response.setSuccess("Bulk create completed: " + 
                          std::to_string(response.successCount) + " succeeded, " +
                          std::to_string(response.failureCount) + " failed");
    ...
    try {
        ...
        switch (request.format) {
            case ExportMetadataRequest::ExportFormat::JSON:
                exportResult = exportToJson(docsToExport, request.outputPath, 
                                           request.compressOutput);
                break;
                
            case ExportMetadataRequest::ExportFormat::CSV:
                exportResult = exportToCsv(docsToExport, request.outputPath,
                                         request.compressOutput);
                break;
...
ImportMetadataResponse MetadataApi::importMetadata(const ImportMetadataRequest& request) {
    ...
    try {
        ...
        response.setSuccess("Import completed: " +
                          std::to_string(response.documentsImported) + " imported, " +
                          std::to_string(response.documentsUpdated) + " updated, " +
                          std::to_string(response.documentsSkipped) + " skipped, " +
                          std::to_string(response.documentsFailed) + " failed");
...
std::vector<metadata::DocumentMetadata> MetadataApi::applyFilter(
    ...
    for (const auto& doc : documents) {
        ...
        if (matches && filter.pathPrefix && 
            doc.info.filePath.find(*filter.pathPrefix) != 0) {
            matches = false;
        }
        
        // Apply date filters
        if (matches && filter.modifiedAfter && 
            doc.info.modifiedTime < *filter.modifiedAfter) {
            matches = false;
        }
        
        if (matches && filter.modifiedBefore && 
            doc.info.modifiedTime > *filter.modifiedBefore) {
            matches = false;
        }
...
Result<std::vector<metadata::DocumentMetadata>> MetadataApi::importFromJson(const std::string& path) {
    try {
        ...
        for (const auto& item : j) {
            ...
            if (item.contains("id")) doc.info.id = item["id"];
            if (item.contains("title")) doc.info.fileName = item["title"];
            if (item.contains("path")) doc.info.filePath = item["path"];
            if (item.contains("contentType")) doc.info.mimeType = item["contentType"];
            if (item.contains("fileSize")) doc.info.fileSize = item["fileSize"];
            if (item.contains("contentHash")) doc.info.sha256Hash = item["contentHash"];
...
Result<std::vector<metadata::DocumentMetadata>> MetadataApi::importFromCsv(const std::string& path) {
    try {
        std::ifstream file(path);
        if (!file) {
            return Error{ErrorCode::FileNotFound, "Failed to open input file"};
        ...
        while (std::getline(file, line)) {
            ...
            while (std::getline(ss, field, ',')) {
                switch (fieldIndex) {
                    case 0: doc.info.id = std::stoll(field); break;
                    case 1: doc.info.fileName = field; break;
                    case 2: doc.info.filePath = field; break;
                    case 3: doc.info.mimeType = field; break;
                    case 4: doc.info.fileSize = std::stoull(field); break;
                    case 5: doc.info.sha256Hash = field; break;
                }
                fieldIndex++;
            }
...
std::vector<ValidateMetadataResponse::ValidationError> MetadataApi::validateFields(
    ...
    if (!metadata.info.filePath.empty() && metadata.info.filePath[0] != '/') {
        errors.push_back({"filePath", "Path must be absolute", "Use absolute path starting with /"});
    }
    
    return errors;
}

void MetadataApi::updateStatistics(const std::string& operationType,
                                   bool success,
...
void MetadataApi::logOperation(const std::string& operation,
...
bool MetadataApi::isHealthy() const {
    ...
    if (stats_.totalRequests > 100) {
        float errorRate = static_cast<float>(stats_.failedRequests) / 
                         static_cast<float>(stats_.totalRequests);
...
std::future<UpdateMetadataResponse> MetadataApi::updateMetadataAsync(const UpdateMetadataRequest& request) {
    return std::async(std::launch::async, [this, request]() {
        return updateMetadata(request);
    });
}

std::future<DeleteMetadataResponse> MetadataApi::deleteMetadataAsync(const DeleteMetadataRequest& request) {
    return std::async(std::launch::async, [this, request]() {
...
std::future<QueryMetadataResponse> MetadataApi::queryMetadataAsync(const QueryMetadataRequest& request) {
    return std::async(std::launch::async, [this, request]() {
        return queryMetadata(request);
    });
}

std::future<ExportMetadataResponse> MetadataApi::exportMetadataAsync(const ExportMetadataRequest& request) {
    return std::async(std::launch::async, [this, request]() {
        return exportMetadata(request);
    });
}

std::future<ImportMetadataResponse> MetadataApi::importMetadataAsync(const ImportMetadataRequest& request) {
    return std::async(std::launch::async, [this, request]() {
...
BulkUpdateResponse MetadataApi::bulkUpdate(const BulkUpdateRequest& request) {
    ...
    try {
        ...
        response.allSucceeded = (response.failureCount == 0);
        response.setSuccess("Bulk update completed: " + 
                          std::to_string(response.successCount) + " succeeded, " +
                          std::to_string(response.failureCount) + " failed");
...
BulkDeleteResponse MetadataApi::bulkDelete(const BulkDeleteRequest& request) {
    ...
    try {
        ...
        response.allSucceeded = (response.failureCount == 0);
        response.setSuccess("Bulk delete completed: " + 
                          std::to_string(response.successCount) + " succeeded, " +
                          std::to_string(response.failureCount) + " failed");
    ...
    try {
        ...
        for (const auto& doc : filteredDocs) {
            ...
            if (request.includeSizeStats) {
                if (response.sizeStats.minSize == 0 || doc.info.fileSize < response.sizeStats.minSize) {
...
std::unique_ptr<MetadataApi> MetadataApiFactory::create(
