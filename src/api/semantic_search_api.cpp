    ...
    Result<ProcessingResult> processDocument(
        ...
        try {
            ...
            if (!embedder_->isInitialized()) {
                ...
                if (!init_result) {
                    return Result<ProcessingResult>(
                        Error{ErrorCode::InternalError, "Failed to initialize embedding generator"});
                }
            }
            ...
            return Result<ProcessingResult>(
                Error{ErrorCode::InternalError, "Document processing failed: " + std::string(e.what())});
        }
    }
...
private:
    ...
    DocumentMetadata enrichMetadata(const std::string& text, const DocumentMetadata& original) {
        ...
        enriched.content_quality = std::min(1.0f, 
            static_cast<float>(enriched.word_count) / 100.0f);
...
public:
    ...
    Result<std::string> ingestDocument(
        const std::string& content,
        const DocumentMetadata& metadata) {
        ...
        for (size_t i = 0; i < result.chunk_ids.size(); ++i) {
            auto add_result = search_engine_->addDocument(
                result.chunk_ids[i],
                content.substr(0, 1000), // First 1000 chars as preview
                result.embeddings[i],
                {{"parent_doc", result.document_id}});
    ...
    Result<SearchResponse> search(const SearchRequest& request) {
        ...
        try {
            ...
            // Perform search
            auto search_result = search_engine_->search(
            ...
            for (const auto& hybrid_result : search_result.value()) {
                ...
                if (request.options.include_snippets) {
                    api_result.snippets = result_utils::generateSnippets(
                        api_result.content,
            ...
            response.total_pages = (response.total_results + request.results_per_page - 1) / request.results_per_page;
...
SemanticSearchAPI::SemanticSearchAPI(
    std::shared_ptr<search::HybridSearchEngine> search_engine,
    std::shared_ptr<vector::DocumentChunker> chunker,
    std::shared_ptr<vector::EmbeddingGenerator> embedder,
    const SearchAPIConfig& config)
    : pImpl(std::make_unique<Impl>(std::move(search_engine), 
                                   std::move(chunker),
                                   std::move(embedder),
                                   config)) {}
...
Result<std::string> SemanticSearchAPI::ingestDocument(
    const std::string& content,
    const DocumentMetadata& metadata) {
...
Result<std::vector<std::string>> SemanticSearchAPI::getSessionHistory(const std::string& session_id) {
...
float calculateComplexity(const std::string& query) {
    ...
    std::regex word_regex(R"(\b\w+\b)");
    auto word_count = std::distance(
        std::sregex_iterator(query.begin(), query.end(), word_regex),
        std::sregex_iterator());
...
std::vector<TextSnippet> generateSnippets(
    const std::string& content,
    ...
    for (size_t match_pos : match_positions) {
        ...
        for (size_t used_pos : used_positions) {
            if (std::abs(static_cast<int>(match_pos) - static_cast<int>(used_pos)) < static_cast<int>(snippet_length / 2)) {
                too_close = true;
                break;
            }
        }
...
std::string highlightTerms(
    ...
    for (const auto& term : sorted_terms) {
        std::regex term_regex("\\b" + std::regex_replace(term, std::regex("[.*+?^${}()|[\\]\\\\]"), "\\$&") + "\\b",
...
std::string generateSummary(const std::string& content, size_t max_length) {
    ...
    while (cut_pos > max_length / 2 && content[cut_pos] != '.' && 
           content[cut_pos] != '!' && content[cut_pos] != '?') {
        cut_pos--;
    }
