...
private:
    ...
    // std::shared_ptr<indexing::TextIndexer> textIndexer_;     // TODO: Add when indexing is implemented
    std::unique_ptr<crypto::IContentHasher> hasher_;
    
public:
    RawTextStorage(
        const RawTextStorageConfig& config,
    ...
    Result<std::string> storeText(const std::string& text, const RawTextMetadata& metadata) override {
        try {
            // Check size limit
            ...
            if (id.empty()) {
                ...
                hasher_->init();
                hasher_->update(std::span{reinterpret_cast<const std::byte*>(text.data()), text.size()});
                id = hasher_->finalize();
            }
    ...
    Result<RawTextEntry> retrieve(const std::string& id) override {
        try {
            ...
            return Error{ErrorCode::Unknown, std::string("Failed to retrieve raw text: ") + e.what()};
        }
    }
    ...
    Result<std::vector<RawTextEntry>> fuzzySearch(const std::string& query, float threshold) override {
        try {
    ...
    Result<std::vector<RawTextEntry>> searchByKeyword(const std::string& keyword, size_t limit) override {
    ...
    Result<std::vector<RawTextEntry>> searchByMetadata(const std::unordered_map<std::string, std::string>& criteria) override {
