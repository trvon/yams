...
std::filesystem::path FilesystemBackend::getObjectPath(std::string_view key) const {
    // Use hash-based sharding for better filesystem performance
    // This ensures consistent sharding regardless of key format
    std::string keyStr(key);

    // Hash the key to get consistent sharding prefix
    ...
    // Hash is already a hex string, use first 4 chars for two-level sharding
    if (hash.length() >= 4) {
        // Create two-level sharding: XX/YY/original_key
        // where XX and YY are from the hash, not the key itself
        return basePath_ / "objects" / hash.substr(0, 2) / hash.substr(2, 2) / keyStr;
    }
