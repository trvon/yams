...
Database::Database(const std::filesystem::path& path) : path_(path.string()) {
    ...
    trace_reference_db_lifetime("open", this, path_, db_, SQLITE_OK, count_live_statements(db_));
}
...
void Database::execute(const std::string& sql) {
    ...
    for (int attempt = 0; attempt < kMaxRetries; ++attempt) {
        ...
        // Check for transient lock errors
        if ((rc == SQLITE_BUSY || rc == SQLITE_LOCKED) && attempt + 1 < kMaxRetries) {
            spdlog::debug("[ReferenceDB] transient lock, retry {}/{}", attempt + 1, kMaxRetries);
            std::this_thread::sleep_for(backoff);
            backoff *= 2;
            continue;
        }
...
void Database::executeFile(const std::filesystem::path& path) {
...
// Backup database
void Database::backup(const std::filesystem::path& destPath) {
    const auto dest = destPath.string();
    sqlite3* destDb = nullptr;
    int rc = sqlite3_open(dest.c_str(), &destDb);
    if (rc != SQLITE_OK) {
        std::string errMsg = destDb ? sqlite3_errmsg(destDb) : "Failed to open backup destination";
        if (destDb) {
...
StatementCache::StatementCache(Database& db) : db_(db) {}

// Returns a copy/move of the cached statement with reset bindings
Statement StatementCache::get(const std::string& key, const std::string& sql) {
