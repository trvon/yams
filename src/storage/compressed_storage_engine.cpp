...
public:
    Impl(std::shared_ptr<StorageEngine> underlying, Config config)
        ...
        , policy_(config_.policyRules)
        , compressionEnabled_(config_.enableCompression)
        , shutdownFlag_(false) {
    ...
    Result<void> store(std::string_view hash, std::span<const std::byte> data) {
        ...
        struct SimpleDecision {
            ...
            compression::CompressionAlgorithm algorithm = compression::CompressionAlgorithm::Zstandard;
            uint8_t level = 3;
        };
        ...
        if (!compressedResult) {
            spdlog::warn("Compression failed for hash {}, storing uncompressed: {}", 
                hash, compressedResult.error().message);
            return underlying_->store(hash, data);
        }
    ...
    Result<bool> exists(std::string_view hash) const noexcept {
        return underlying_->exists(hash);
    }
    
    Result<void> remove(std::string_view hash) {
        return underlying_->remove(hash);
    }
    ...
    // Note: list(), stats(), and compact() methods removed as they're not part of the StorageEngine interface
    
    StorageStats getStats() const noexcept {
        return underlying_->getStats();
    }
    
    Result<uint64_t> getStorageSize() const {
        return underlying_->getStorageSize();
    }
    ...
    std::future<Result<void>> storeAsync(std::string_view hash, std::span<const std::byte> data) {
        ...
        return std::async(std::launch::async, [this, hash = std::string(hash), data = std::vector<std::byte>(data.begin(), data.end())] {
    ...
    std::future<Result<std::vector<std::byte>>> retrieveAsync(std::string_view hash) const {
        return std::async(std::launch::async, [this, hash = std::string(hash)] {
            return this->retrieve(hash);
        });
    }
    
    // Batch operations
    std::vector<Result<void>> storeBatch(const std::vector<std::pair<std::string, std::vector<std::byte>>>& items) {
    ...
    Result<size_t> triggerCompressionScan() {
        ...
        return Error(ErrorCode::NotImplemented, "Background compression scan requires list() functionality");
    }
    
    bool waitForAsyncOperations(std::chrono::milliseconds timeout) {
        std::unique_lock lock(asyncMutex_);
        return asyncCV_.wait_for(lock, timeout, [this] {
            return asyncQueue_.empty() && activeJobs_ == 0;
        });
    }
...
private:
    ...
    Result<std::vector<std::byte>> compressData(std::span<const std::byte> data, const DecisionType& decision) {
        auto& registry = compression::CompressionRegistry::instance();
        auto compressor = registry.createCompressor(decision.algorithm);
        if (!compressor) {
            return Result<std::vector<std::byte>>(Error{ErrorCode::InvalidArgument, "Compressor not available"});
        }
    ...
    Result<std::vector<std::byte>> decompressData(std::span<const std::byte> data) const {
        ...
        if (!decompressor) {
            return Result<std::vector<std::byte>>(Error{ErrorCode::InvalidArgument, "Decompressor not available"});
        }
        ...
        actualCRC = calculateCRC32(std::span<const std::byte>{
            reinterpret_cast<const std::byte*>(result.value().data()),
            result.value().size()
        });
        ...
        // Update statistics
        updateStats([&](compression::CompressionStats& stats) {
            stats.algorithmStats[algorithm].recordDecompression(
                compressedData.size(), result.value().size(), duration);
        });
        ...
        while (!shutdownFlag_) {
            ...
            
            {
                ...
                asyncCV_.wait(lock, [this] {
                    return !asyncQueue_.empty() || shutdownFlag_;
                });
...
CompressedStorageEngine::CompressedStorageEngine(
    std::shared_ptr<StorageEngine> underlying,
    Config config)
    : StorageEngine({.basePath = "/tmp/compressed_storage_dummy"})  // Dummy config since we delegate to underlying
    , pImpl(std::make_unique<Impl>(std::move(underlying), std::move(config))) {
}

CompressedStorageEngine::~CompressedStorageEngine() = default;

Result<void> CompressedStorageEngine::store(std::string_view hash, std::span<const std::byte> data) {
...
// Async operations
std::future<Result<void>> CompressedStorageEngine::storeAsync(std::string_view hash, std::span<const std::byte> data) {
    return pImpl->storeAsync(hash, data);
}

std::future<Result<std::vector<std::byte>>> CompressedStorageEngine::retrieveAsync(std::string_view hash) const {
    return pImpl->retrieveAsync(hash);
}

// Batch operations
std::vector<Result<void>> CompressedStorageEngine::storeBatch(const std::vector<std::pair<std::string, std::vector<std::byte>>>& items) {
