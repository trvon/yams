...
Result<void> syncFile(const std::filesystem::path& path) {
    HANDLE hFile =
        ::CreateFileW(path.wstring().c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
Result<void> atomicWriteOptimized(const std::filesystem::path& path,
    ...
    // Write to temp file
    HANDLE hFile = ::CreateFileW(tempFile, GENERIC_WRITE,
                                 0, // No sharing
...
Result<void> enableCopyOnWrite(const std::filesystem::path& path) {
    ...
    if (::GetVolumeInformationW(root.c_str(), nullptr, 0, nullptr, nullptr, &fsFlags, fsName,
                                MAX_PATH)) {
        // Check if this is ReFS
        if (std::wstring(fsName) == L"ReFS") {
            // Enable integrity streams for better data protection
            HANDLE hFile = ::CreateFileW(path.wstring().c_str(), GENERIC_READ | GENERIC_WRITE, 0,
                                         nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
            ...
            if (hFile != INVALID_HANDLE_VALUE) {
                ...
                ::DeviceIoControl(hFile, FSCTL_SET_SPARSE, &sparseBuffer, sizeof(sparseBuffer),
