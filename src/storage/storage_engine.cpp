...
struct StorageEngine::Impl {
    ...
    explicit Impl(StorageConfig cfg) 
        : config(std::move(cfg)),
          writeMutexPool(config.mutexPoolSize) {
...
StorageEngine::StorageEngine(StorageConfig config) 
    ...
    spdlog::debug("Initialized storage engine at: {}", 
                  pImpl->config.basePath.string());
}
...
std::filesystem::path StorageEngine::getObjectPath(std::string_view hash) const {
    ...
    if (hash.ends_with(".manifest")) {
        ...
        return pImpl->config.basePath / "manifests" / std::string(prefix) / (std::string(suffix) + ".manifest");
    }
...
Result<void> StorageEngine::ensureDirectoryExists(
...
Result<void> StorageEngine::atomicWrite(
...
Result<void> StorageEngine::store(
...
Result<std::vector<std::byte>> StorageEngine::retrieve(std::string_view hash) const {
    ...
    bool isManifest = hash.ends_with(".manifest");
    if (!isManifest && hash.length() != HASH_STRING_SIZE) {
        spdlog::error("Invalid hash length for retrieve: expected {} characters, got {} for hash '{}'", 
                     HASH_STRING_SIZE, hash.length(), hash);
...
Result<bool> StorageEngine::exists(std::string_view hash) const noexcept {
    try {
        ...
        bool isManifest = hash.ends_with(".manifest");
        if (!isManifest && hash.length() != HASH_STRING_SIZE) {
            spdlog::error("Invalid hash length for exists: expected {} characters, got {} for hash '{}'", 
                         HASH_STRING_SIZE, hash.length(), hash);
...
Result<void> StorageEngine::remove(std::string_view hash) {
    ...
    bool isManifest = hash.ends_with(".manifest");
    if (!isManifest && hash.length() != HASH_STRING_SIZE) {
        spdlog::error("Invalid hash length for remove: expected {} characters, got {} for hash '{}'", 
                     HASH_STRING_SIZE, hash.length(), hash);
        return Result<void>(ErrorCode::InvalidArgument);
    }
...
std::future<Result<void>> StorageEngine::storeAsync(
    std::string_view hash,
    std::span<const std::byte> data) {
...
std::future<Result<std::vector<std::byte>>> StorageEngine::retrieveAsync(
    std::string_view hash) const {
    
    std::string hashCopy(hash);
    
    return std::async(std::launch::async,
        [this, hash = std::move(hashCopy)]() {
...
Result<void> StorageEngine::verify() const {
    ...
    try {
        ...
        return errorCount == 0 ? Result<void>{} : 
               Result<void>(ErrorCode::CorruptedData);
...
std::filesystem::path AtomicFileWriter::generateTempName(
